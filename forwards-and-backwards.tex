...
\comment{Not sure next sentence strictly true - probably assuming a bit
more - certainly some regularity.}
...
as we check in Lemma~\ref{old 6.1}, \comment{this is referring to the
old Lemma 6.1, now to go in Appendix,}
...
purely atomic measures, with atoms of mass $1/N$, which is
tight 
...
\end{proposition}
The example that we have in mind for the kernel $\rho_gamma$ is a Gaussian
kernel.
\begin{remark}
...
begun at spatial location $L_0^N = x$,
can be obtained by picking a focal individual uniformly at 
random from the population at $x$,
\comment{actually there will be at most one individual at any point $x$}
...
any fixed value converges, Theorem~\ref{}. 
...
\comment{Maybe check for more recent references for paragraph below.}
...
\comment{I don't understand this - something has gone wrong.}
...
$\rho_\epsilon=p_{\epsilon^2}$ is preserved under the dynamics dictated by $q_{\theta}$.
...
Write ${\mathcal C}={\mathcal C}(\IR^d\times [0,\infty)$ for the point measures 
on $\IR^d\times [0,\infty)$ and ${\mathcal C}_N$ for the subset consisting of 
counting measures on $\IR^d\times [0,N]$.
%We say that a ${\mathcal D}([0,\infty), {\mathcal C})$-valued process 
%$(\xi_t)_{t \ge 0}$
%    is a solution to the $(A, \lp_0)$ martingale problem
%    if %it has initial distribution $\lp_0$ and 
%$f(\lp_t) - f(\lp_0)-\int_0^t Af(\lp_s) ds$ is a martingale (with 
...
by
%and $\lp_0 \in {\mathcal C}_N$, %\mathcal{M}_F(\IR^d \times [0,N])$,
...
as before. 
Given $\lp_0 \in {\mathcal C}_N$, %\mathcal{M}_F(\IR^d \times [0,N])$,
%we define $(\lp^{N}_t)_{t \geq 0}$
we say that a ${\mathcal D}([0,\infty), {\mathcal C}_N)$-valued process 
$(\xi_t)_{t \ge 0}$
   is a solution to the $(A, \lp_0)$ martingale problem
    if %it has initial distribution $\lp_0$ and 
$f(\lp_t) - f(\lp_0)-\int_0^t Af(\lp_s) ds$ is a martingale (with 
...
Elsewhere we have written $\eta^N$ for the spatial projection of the lookdown process $\lp^N$,
although $\eta^N$ was defined first as a solution to a martingale problem.
For the purposes of this result --
the application of the Markov Mapping Theorem \citep[Thm A.2 in][]{etheridge/kurtz:2018} --
we must disambiguate these, and so define the spatial projection
maps $\gamma^N: \mathcal{M}(\IR^d \times [0, N]) \to \mathcal{M}(\IR^d)$
by $\gamma^N(\xi^N)(\cdot) = \xi^N(\cdot \times [0, N]) / N$,
and $\gamma: \mathcal{M}(\IR^d \times [0, \infty)) \to \mathcal{M}(\IR^d)$
by $\gamma(\xi)(\cdot) = \lim_{u_0 \to \infty} \xi(\cdot \times [0, u_0]) / u_0$.
We will also need an inverse notion:
for a measure $\lp^N$ on $\IR^d \times [0, N]$ and a sigma-field $\mathcal{F}$,
we say that \emph{$\lp$ is conditionally uniform given $\mathcal{F}$}
if for all compactly supported $f$,
\begin{align} \label{eqn:conditionally_uniform}
    \IE[ e^{-\langle f, \lp \rangle} \;|\; \mathcal{F} ]
    &=
    e^{-\langle F_f^N, \gamma(\lp) \rangle} ,
\end{align}
where
$$ F_f^N(x) = - N \log \frac{1}{N} \int_0^N e^{-f(x, u)} du . $$
(In other words, the $[0, N]$ components of $\lp$
are independent, uniformly distributed on $[0, N]$, and independent of $\gamma(\lp)$.)
...
\begin{align} \label{eqn:conditionally_poisson}
    \IE[ e^{-\langle f, \lp \rangle} \;|\; \mathcal{F} ]
    &=
    e^{-\langle \int_0^\infty (1 - e^{-f(x, u)}) du, \gamma(\lp)(dx) \rangle} .
\end{align}
(In other words, $\lp$ is conditionally Poisson with Cox measure $\gamma(\lp) \times \Lambda$.)

\begin{proposition}
    \label{thm:mmt_application}
    Let $\mu_0$ be a probability distribution on $\measures$,
    and $\nu_0^N$ a probability distribution on $\mathcal{M}(\IR^d \times [0, N])$
    such that a draw $\lp^N_0$ from $\nu_0^N$ is conditionally uniform given $\gamma(\lp^N_0)$
    ...
    with initial distribution $\mu_0$
    then there exists a solution $\lp^N$
    of the martingale problem of Definition~\ref{defn:lookdown_mgale}
    with initial distribution $\nu_0^N$
    such that $\eta^N = \gamma \cdot \lp^N$
    has the same distribution on $D_{\measures}[0, \infty)$
    as $\tilde \eta^N$.
    Furthermore, $\lp^N$ is conditionally uniform given given $\mathcal{F}^{\eta}_t$
    in the sense of \eqref{eqn:conditionally_uniform}.

    \comment{Theorem~\label{thm:nonlocal_convergence}
        isn't written as ``the $(A, \mu)$ martingale problem is'';
        rephrase here or there?}

    Similarly,
    let $\mu_0$ be a probability distribution on $\measures$,
    and $\nu_0$ a probability distribution on $\mathcal{M}(\IR^d \times [0, \infty))$
    such that a draw $\lp_0$ from $\nu_0$ is conditionally Poisson given $\gamma(\lp_0)$
    in the sense of \eqref{eqn:conditionally_poisson}.
    If $\tilde \eta$ is a solution
    of the limiting martingale problem of Theorem~\label{thm:nonlocal_convergence}
    with initial distribution $\mu_0$
    then there exists a solution $\lp$
    of the martingale problem of of Definition~\ref{defn:limiting_lookdown_mgale}
    with initial distribution $\nu_0$
    such that $\eta = \gamma \cdot \lp$
    has the same distribution on $D_{\measures}[0, \infty)$
    as $\tilde \eta$.
...
\begin{proof}[of Proposition~\ref{thm:mmt_application}]
    This is the content of the Markov Mapping Theorem applied to our situation,
    \citet[Thm A.2 in][]{etheridge/kurtz:2018}.
    In the notation of that theorem, $\gamma$ is as above,
    ...
    for all $f \in \mathcal{D}(A^N)$ (and similarly a function $\psi$ for $A$).
    ...
    which we precisely state, and verify, in Appendix~\ref{sec: Markov Mapping Theorem Application}.

    For finite $N$,
    taking $f(\lp)$ of the form \eqref{eqn:test_functions},
    we can use $\psi^N(\lp) = \langle C + u |F(x, \eta)|, \lp(dx, du) \rangle$
    ...
    and so taking for example the mostly troublesome term in $Af(\lp)$,
    ...
    \end{align}
    The second line would be just what we want,
    except that $\psi(\lp)$ cannot depend on $f$, and hence neither on $u_0$.
    So, the third line provides us with the required bound,
    absorbing $\| \partial_u g \|_\infty e^{u_0}$ into $c_f$
    and $\langle |F(x, \eta) u e^{-u}|, \lp \rangle$ into $\psi(\lp)$.
    Furthermore, since $F(x, \eta)$ only depends on spatial location, not level,
    for a given $\lp$ (and hence $\eta = \gamma(\lp)$),
    ...
    \citep[see Condition 2.1 in][]{etheridge/kurtz:2018},
    and our processes fall in this space (by conditional uniformity).
\end{proof}

In this section I haven't specified the filtration for the martingale problem.}
...
the Markov Mapping Theorem \citep[Thm A.2 in][]{etheridge/kurtz:2019}
...
\cite{hallatschek/nelson:2008, durrett/fan:2016, biswas/etheridge/klimek:2018}.
...
\begin{equation*}
    ...
    U_a(\tau)
    +
    v \ell\big(X_a(\tau-), y(X_a(\tau-), z), \eta(\tau-)\big) .
\end{equation*}
...
\begin{remark}
    Although we have a single construction that couples
    the processes across all $N$, the actual 
trajectories, $X_a(\cdot)$, do not necessarily coincide for 
...
and for the moment write $r(x)$ for $r(x, \eta)$,
so that
\begin{align}
\DG(rg)(x) - g(x) \DG(r)(x)
    &= \nonumber
...
For each point of $R_0$, begin a line of descent
...
As a consequence of~(\ref{formal intensity}), writing 
$\mathfrak{p}(t,y,x)$ for
...
\begin{equation}
\label{ptyx}
\mathfrak{p}(t,y,x)=\frac{w(x)}{w(y)}\IE_x\left[\exp\left(\int_0^t b(Y_s)ds\right)
\ind_{Y_t=y}\right].
\end{equation}

To make~(\ref{ptyx}) meaningful, we multiply by suitable test functions and 
integrate.
$$\int\int \psi(y)w(y)\mathfrak{p}(t,y,x)\phi(x)dydx=\int\phi(x)w(x)
...
to $\mathfrak{p}(t,y,x)$), we can write this as 
...
In this section we present formal proofs of our 
three
scaling limits. In
Subsection~\ref{sec:population_density_proof} 
...
limit to obtain a classical PDE. In 
Subsection~\ref{subsec:one step convergence proof}
we prove our third scaling limit, in which 
we pass from our population model 
directly to a reaction diffusion equation without going via a nonlocal
limit.

\comment{TODO add sentences about the sections Peter is editing}
...
second derivatives, there exists a $C_f < \infty$, independent of $N$,
    such that
    \begin{equation}
\label{eta_f_bound}
...
We write
    ...
    \end{align*}
    where $z = \kappa x + (1-\kappa) y$ for some $0 \le \kappa \le 1$. Condition~\ref{control through r} 
...
the resulting bound on $\langle 1,\eta_s\rangle$ into the
...
this in two examples; in the first the nonlocal equation is a reaction
...
q_\theta(y,dz),
%+(-1)^j\Big(1-\frac{F(\rho_\epsilon*\eta_s(x))}{\theta}\Big)\Big),
...
%\Big(1+
(-1)^j\Big(1-\frac{F(\rho_\epsilon*\eta_s(y))}{\theta} %\Big)
\Big),
...
\begin{corollary}[Tightness of $\{(\rho_\epsilon*\eta_t(x) dx)_{t\geq 0}\}$]
...
that 
$\IE[\sup_{0\leq t\leq T} \langle 1,\eta_t\rangle]$ is 
bounded goes through unchanged, and since 
$\langle 1, \rho_\epsilon*\eta_t(x)dx\rangle=\langle 1, \eta_t\rangle$,  
...
Using Corollary~3.7.4 (p.129) of~\cite{ethier/kurtz:1986}, 
to prove tightness
of the sequence of real-valued processes $X_f^N$
it suffices to check compact containment of 
the sequence $\{\rho_\epsilon*\eta_t(x)dx\}_{N\geq 1}$ at any rational time $t$ and
...
+
\|f\|_\infty \int_s^t
%\Big\langle \sum_{k=1}^n|a_k|
%\big(\rho_\epsilon*\eta_u(x)\big)^k, 
\big\langle \big|F\big(\rho_\epsilon*\eta_u(x)\big)\big|,
\eta_u(dx)\big\rangle du +2\sup_{0\leq u\leq T}|\widehat{M}^N(f)_u|.
    \end{multline*}
...
\begin{multline*}
\IE\Big[
\Big(\int_s^t\big\langle \big|F\big(\rho_\epsilon*\eta_u(x)\big)\big|,
\eta_u(dx)\big\rangle du\Big)^2\Big]
\\
=
\IE\Big[
\int_s^t\big\langle \big|F\big(\rho_\epsilon*\eta_u(x)\big)\big|,
\eta_u(dx)\big\rangle 
\int_u^t\big\langle \big|F\big(\rho_\epsilon*\eta_r(x)\big)\big|,
\eta_r(dx)\big\rangle dr
du\Big].
\end{multline*}
Since $F$ is polynomial, using our moment bounds we see that this 
quantity is bounded by $C(t-s)^2$ for some constant $C$ which we can 
take to be independent of $s$, $t$ and $\epsilon$. Markov's inequality 
then gives
\[
\IP\Big[\|f\|_\infty\int_s^t
\big\langle \big|F\big(\rho_\epsilon*\eta_u(x)\big)\big|,
\eta_u(dx)\big\rangle du \geq \frac{\nu}{3}\Big]
\leq C\frac{(t-s)^2}{\nu^2}.
\]
...
\[
\IP\Big[\|\Delta f\|_\infty\, \sup_{0\leq t\leq T} \langle 1,\eta_t\rangle 
>K\Big]<\frac{\nu}{3}, 
\]
and so choosing $\delta$ so that $K\delta<\nu/3$ in this 
expression and $C\delta<T\nu^3/3$ in~(\ref{union bound}),
combining with~(\ref{martingale term to zero}), the result follows.
\end{proof}

\subsubsection{Identification of the limit}
\label{limit in onestep case}


We now turn to the identification of the limit points of the sequence 
$\big\{\big(\rho_\epsilon*\eta_t(x)dx\big)_{t\geq 0}\big\}_{N\geq 1}$.
Since we only know that for every sequence there is a weakly convergent subsequence,
...
%\begin{lemma} \label{ContinuityEstimate}
%Under the conditions of Theorem~\ref{TeoremOneStepConvergence} we have that 
%for all $x \in \IR^d$, $\delta>0$,
%\[
%\int |\rho_\epsilon*\eta_s(x) -\rho_\epsilon*\eta_s(y)| \rho_\delta(x-y) dy \]
%is bounded by
%\comment{Will need to be restated once corrected. Notice we want to put in powers of 
%$\rho_\epsilon*\eta_s(x)$ and $\rho_\epsilon*\eta_s(y)$ }
%\begin{align*}
...
\\ 
&\leq 
%\big\langle |\psi_t^{\epsilon,x}(z)-\psi_t^{\epsilon,y}(z)|, 
...
%\\ 
%&+ C \int_0^t \langle |\psi_{t-s}^{\epsilon,x}(z) - \psi_{t-s}^{\epsilon, y}(z) | 
%\rho_\epsilon*\eta(z), \eta_s(dz) \rangle  ds | 
%\label{eq:bound1CE} 
\end{align} 
%Recalling that $F$ is a polynomial, and 
%using Lemma~\ref{Lemma:BoundPsiHS2} we can then write, 
%\begin{align*}
% \int &|\rho_\epsilon*\eta_s(x) -\rho_\epsilon*\eta_s(y)| \rho_\delta(x-y) dy  
%\leq 
%\int |p_{\epsilon^2+s}(y,z)-p_{\epsilon^2+s}(x,z)|\eta_0(dz) \rho_\delta(x-y)dy 
%\\
%&+ 
%C\int_0^s \langle p_{\epsilon^2+s-r}(y,z)-p_{\epsilon^2+s-r}(x,z)|, \eta_r(dz) \rangle 
%\rho_\delta (x-y) dr  
%\\  
%& + C \int\int_0^s \langle |p_{\epsilon^2+s-r}(y,z) - p_{\epsilon^2+s-r}(x,z) | 
%\sum_{k=0}^na_k\rho_\epsilon*\eta_r(z)^k, \eta_r(dz) \rangle  dr\rho_\delta(y-x)dy    
%\\ & + \int|M_s(y)-M_s(x)| \rho_\delta(y-x)dy  
%\\  & 
%+ \frac{C}{\epsilon^2 \theta} \int \langle \sum_{k=0}^na_k\rho_\epsilon*\eta(z)^k,
%\eta_s(dz)\rangle ds   
%+\mathcal{O}\left( \frac{1}{\epsilon^2 \theta} \right), 
%\end{align*}
%for some non-negative constants $a_k$.
%By Lemma~\ref{} the last integral term is of 
%order $(\epsilon^2 \theta)^{-1}$. 
%Using Lemma~\ref{Lemma:ContinuityHS}, the contribution of the first 
%three terms is bounded by,
%\begin{align*}
%\int &\langle \frac{|y-x|}{\sqrt{\epsilon^2+s}}
%\Big(p_{2(\epsilon^2+s)}(x,z) +p_{2(\epsilon^2+s)}(y,z)\Big) 
%\rho_{\delta}(y-x) dy, \eta_0(dz)\rangle 
%\\
%& + \int \int_0^s  
%\langle \frac{\|y-x\|}{\sqrt{\epsilon^2+s-r}} 
%\Big(p_{2(\epsilon^2+s-r)}(x,z) +p_{2(\epsilon^2+s-r)}(y,z)\Big) 
%\rho_\delta(x-y) dy 
%\sum_{k=0}^n\rho_\epsilon*\eta_r(z)^k, \eta_r(dz) dr 
%\\ 
%& +  \int \int_0^s \langle \frac{|y-x| \rho_\epsilon*\eta(z)}{\sqrt{\epsilon^2+s-r}}, \eta_r(dz) \rangle \rho_\delta(y-x) dy dr \\ 
%& \leq 
%\frac{\delta}{\sqrt{\epsilon^2+s}} %\langle 1, \eta_0(dz) \rangle 
%+ \int_0^s \frac{\delta}{\sqrt{\epsilon^2+s-r}} 
%\Big(p_{2(\epsilon^2+s-r)}(x,z) +p_{2(\epsilon^2+s-r+\delta)}(y,z)\Big) 
%\sum_{k=0}^n a_k\rho_\epsilon*\eta_r(z)^k,
%\eta_r(dz) \rangle dr.
%\end{align*}
...
\lim_{N\to\infty}\Big|\int_0^t \IE\Big[\big\langle T_{\epsilon^2} f(y) 
...
\end{equation}
%is of order
%\comment{Equation below needs to be filled in when we have the proof}
%\begin{equation*}
...
and $N$. Since $\delta$ is arbitrary, the result will follow.
...
\leq 
|\rho_\epsilon*\eta_s(y)-\rho_\epsilon*\eta_s(w)|
\sum_{k=1}^{n-1}|b_k|\left(\rho_\epsilon*\eta_s(y)^k+\rho_\epsilon*\eta_s(w)^k\right).
\]
...
\begin{multline*}
\int_\delta^t\IE\Big[\big\langle\frac{\|y-w\|}{\sqrt{s+\epsilon^2}}
...
\IE\Big[\big\langle
p_{2(s+\epsilon^2)}(y,z)   %+p_{2(s+\epsilon^2)}(w,z)\big)
,\eta_0(dz)
\big\rangle
\rho_\epsilon*\eta_s(y)^k %+\rho_\epsilon*\eta_s(w)^k\big)
...
\big\rangle
\rho_\epsilon*\eta_s(y)^k %+\rho_\epsilon*\eta_s(w)^k\big)
p_{\epsilon^2}(x,y)dy \rho_\epsilon*\eta_s(x) dy dx ds\big\rangle ds \Big]
\\
\leq
\int_\delta^t\frac{\epsilon}{\sqrt{s+\epsilon^2}}
\IE\Big[\big\langle
...
p_{\epsilon^2}(x,y) dy dx \big\rangle ds,
\end{multline*}
...
$ %\[
\big\langle
p_{2(s+\epsilon^2)}(w,z)   %+p_{2(s+\epsilon^2)}(w,z)\big)
,\eta_0(dz)
\big\rangle
...
from this term of the form $C\epsilon/\delta^{(d+1)/2}$.
Note that for fixed $\delta$, this bound tends to zero as $\epsilon\to 0$.

Now consider
\begin{multline*}
\int_\delta^t\int_0^{s-\delta}\Big\langle\frac{\|y-x\|}{\sqrt{s-r+\epsilon^2}}
\big\langle\big(p_{2(s-r+\epsilon^2)}(y,z)+ p_{2(s-r+\epsilon^2)}(w,z)\big)
\rho_\epsilon*\eta_r(z)^j,\eta_r(dz)\big\rangle dr
\\
\rho_\epsilon*\eta_s(y)^kp_{\epsilon^2}(y,w),\eta_s(dw)
\Big\rangle ds
\\
\leq C\epsilon
\int_\delta^t\int_0^{s-\delta}\Big\langle\frac{1}{\sqrt{s-r+\epsilon^2}}
\big\langle
\big(p_{2(s-r+\epsilon^2)}(y,z)+ p_{2(s-r+\epsilon^2)}(w,z)\big)
\rho_\epsilon*\eta_r(z)^j,\eta_r(dz)\big\rangle dr
\\
\rho_\epsilon*\eta_s(y)^kp_{2\epsilon^2}(y,w),\eta_s(dw)
\Big\rangle ds,
\end{multline*}
...
+\IE\Big[\int p_{2(s-r)}(y,x)\rho_\epsilon*\eta_r(x)\rho_\epsilon*\eta_r(y)^{k+1} dy dx\Big]
\\
\leq
\IE\Big[\big\langle \rho_\epsilon*\eta_r(z)^{j+k+1}, \eta_r(dz)\rangle\Big]
...
It remains to control the contribution arising from the martingale terms 
...
The other terms follow in a similar way.
...
$\big\{\big(\rho_\epsilon*\eta_t(dx)\big)_{t\geq 0}\big\}$ 
converging to a limit point $\big(u_t(x)dx\big)_{t\geq 0}$, then
\[ 
\int f(x) \rho_\epsilon * \eta_t(x) F(\rho_\epsilon * \eta_t(x)) dx 
\rightarrow \int f(x) u_t(x) F(u_t(x)) dx. 
\]
...
\int f(x) \rho_\epsilon * \eta_t(x) \rho_\epsilon * \eta_t(x)^2 dx 
\rightarrow \int f(x) u_t(x) u_t(x)^2 dx. 
\end{equation}
The converge of higher powers will follow in an entirely analogous manner, but
with more complex expressions.

The approach is standard. We fix $\delta>0$ and in keeping with
our notation $\rho_\epsilon$ in this subsection, use $\rho_\delta$ to 
denote the symmetric Gaussian kernel with variance parameter $\delta^2$.
Our strategy is to show that, up to an error of $\mathcal{O}(\delta)$,
\begin{equation}
\label{aprox2}
\int f(z) \rho_\epsilon*\eta_s(z)^2 dz
%(\eta_s * \rho_\epsilon)^2 \rangle 
\sim 
\int \int f(z) (\rho_\epsilon * \eta_s)(z) 
\rho_\delta (z-y) (\rho_\epsilon * \eta_s)(y) dz dy.
\end{equation}
Analogously, also up to an error of $\mathcal{O}(\delta)$,
\begin{equation}
\label{aprox3}
\int f(z) u_s(z)^2 dz
%(\eta_s * \rho_\epsilon)^2 \rangle 
\sim 
\int \int f(z) u_s(z) 
\rho_\delta (z-y) u_s(y) dz dy.
\end{equation}
On the other hand,
weak convergence of $\rho_\epsilon * \eta$ 
(plus continuity of the mapping $(z,y) \rightarrow f(z) \rho_\delta(z-y)$) 
gives that 
\begin{equation}
\int \int f(z) (\rho_\epsilon * \eta_s)(z) 
\rho_\delta (z-y) (\rho_\epsilon * \eta_s)(y) dz dy
...
\begin{align}
&\int_0^t 
\mathbb{E}\left[| \langle f(y), (\eta_s * \rho_\epsilon(y))^2) dy \rangle 
- \int \int f(y) (\rho_\epsilon * \eta_s)(z) \rho_\delta (z-y) 
(\rho_\epsilon * \eta_s)(y) dz dy. |\right] ds 
\nonumber 
\\ 
&\leq  
\Vert f \Vert_\infty \int  \int_0^t  
\mathbb{E}\left[ \int \left\{ \left|(\rho_\epsilon * \eta_s)(y)
-(\rho_\epsilon * \eta_s)(z) \right|   \rho_\delta(z-y) dz \right\} 
...
\comment{I haven't turned the $\mu$ into $\mu_\theta$ below this point.}
...
follows a similar pattern as the the proofs of convergence for the population process
...
\comment{TODO: harmonize notation with above}
$$
\dot u 
=
c^N(x, \eta)
...
where
\begin{align*}
    c^N(x, \eta) &= \frac{\theta}{N} \gamma(x, \eta) \int_{\IR^d} r(y, \eta) q(x, dy) \\
    \text{and}\qquad
    b^N(x, \eta)
    &=
    ...
    + F(x, \eta) 
\end{align*}
(see equations \eqref{eqn:level_generator} and \eqref{eqn:b_limit}), where
$c^N(x, \eta) \ge 0$ and $b^N(x, \eta)$ may take either sign.
Assumptions~\ref{def:model_setup} imply that $c^N(x, \eta)$ is bounded
and $b^N(x, \eta)$, because of $F(x, \eta)$, is bounded above but not necessarily below.
In \citet{kurtz/rodrigues:2011}, $b^N$ was bounded above and $c^N$ was bounded away from zero,
so they noted that if $U_a(t) \ge b^N/c^N$
...
In our case $b^N$ and $c^N$ depend on the state of the population,
so we have no such monotonicity.

To complete the description of the process,
each line of descent
gives birth to lines at higher levels at rate $2 (N - u) c^N(x, \eta)$,
...
\begin{align} \label{eqn:f_xi_qv}
    ...
    &=
    \int_0^t
    \bigg \langle c^N(x, \eta_s^N) \int_u^N \int_{\IR^d}
        \big[ f(y, u_1)^2
\\&\qquad \qquad {}
...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
    ...
    therefore, compact containment follows from Lemma~\ref{lem:compact containment lookdown}.

    Theorem~3.9.1 of \citet{EK} requires that $\{(F(\lp^N_t))_{t \ge 0}\}_N$
    ...
    by the Stone-Weierstrass theorem, the algebra they generate is dense in $C_0(\lpmeasures)$,
    the continuous functions that vanish at infinity
    (i.e., are less than $\epsilon$ outside of a compact set, for any $\epsilon$).
    Since $C_0(\lpmeasures)$ is dense in $\bar C(\lpmeasures)$
...
\begin{lemma}
    ...
    Let $h$ be a positive, continuous, nondecreasing, differentiable function on $[0, \infty)$
    such that
    $\int_0^\infty \int_u^\infty h(v) dv du$,
    $\int_0^\infty u^2 |h'(u)| du$,
    and $\int_0^\infty h(u)^2 du$
    are all finite,
    Then under the assumptions of Lemma~\ref{lem:compact containment lookdown},
    for any $T$ there exists a constant $K$ such that for all $M > 0$,
    $$ 
...
We postpone the proof of this Lemma to show how it gets us to compact containment.
First, we show that this implies compact containment for arbitrary
compactly supported functions.

\begin{lemma}
    \label{lem:compact containment hn}
    Under the assumptions of Lemma~\ref{lem:compact containment lookdown},
    for any $T$ and $k$ there exists a constant $K_k$ such that for all $M > 0$,
    $$ 
    \limsup_{N \to \infty} \IP\left\{ \sup_{0 \le t \le T} \langle h_k, \lp^N_t \rangle > M \right\}
    <
    \frac{K_k}{M} .
    $$
\end{lemma}
...
\begin{proof}[of Lemma~\ref{lem:compact containment hn}]
    ...
    For any $f \in C(\bar \IR^d \times [0, \infty))$ with compact support,
    there is a $c_f < \infty$ such that $f(x, u) \le c_f h(u)$.
...
\begin{lemma}[Compact containment for $\lp$]
    \label{lem:compact containment lookdown}
    Under Assumptions~\ref{def:model_setup},
    if $\theta/N \to \alpha$ and $\lp_0^N \to \lp_0$ weakly as $N \to \infty$
    \comment{and they are conditionally Uniform with the limit conditionally Poisson},
    ...
    \limsup_{N \to \infty}
    \IP\left\{
        \sup_{0 \le t \le T} \langle h_k, \lp^N_t \rangle > C_n
        \text{ for some } k \ge 1
    \right\}
    < \delta .
    \end{align}
...
\begin{proof}[of Lemma~\ref{lem:compact containment lookdown}]
    ...
    $$ 
    \IP\left\{
        \sup_{0 \le t \le T} \langle h_k, \lp^N_t \rangle > C_k
        \text{ for some } k \ge 1
    \right\}
    \le
    \sum_{k \ge 1}
    ...
    by taking $C_k = 2^{k-1} K_k / \delta$ and using Lemma~\ref{lem:compact containment hn}.
\end{proof}

Finally, we prove the key lemma.

\begin{proof}[of Lemma~\ref{lem:compact containment hn}]
    \comment{(TODO: shorten and make constants have reasonable notation)}
    ...
    \begin{align*}
        ...
        ds ,
    \end{align*}
    where there is no supremum before the integral because the integrand is nonnegative.
    ...
    \\ & \le
        C'' e^{C_1 T} ,
    \end{align*}
    for a constant $C''$ which is finite
...
\begin{proof}[of Lemma \ref{lem:test_fn_tightness}]
    ...
    Indeed, the proof follows exactly as the proof of
    Lemma~\ref{lem:eta_projections_tightness},
    except that terms are bounded by constant multiples of $\langle h, \xi^N_s \rangle$
    instead of $\langle 1, \eta^N_s \rangle$.
    \comment{Give an example of this.}
    There is only one term that might use explanation,
    which is
    \begin{align*}
        &
        \frac{\theta}{N} \int_{\IR^d} \int_u^N (f(y, u) - f(x, u)) du_1 q^m(x, dy, \eta^N_s)
        \\ &\qquad
        % = \frac{\theta}{N} (N - u) \int_{\IR^d} (f(y, u) - f(x, u)) q^m(x, dy, \eta^N_s) \\ &\qquad
        = \left(1 - \frac{u}{N}\right) \theta \int_{\IR^d}
            (f(y, u) - f(x, u)) q^m(x, dy, \eta^N_s) ,
    \end{align*}
    but this is now bounded as in the proof of Lemma~\ref{lem:eta_f_bound}.
\end{proof}
