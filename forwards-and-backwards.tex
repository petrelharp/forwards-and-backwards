...
thus scaling the kernels appearing in $r$, $\gamma$, and $F$ and making interactions are pointwise,
...
that number remains finite; furthermore.
On the other hand, we shall consider a scaling in which 
...
It will be convenient to work with $F$ instead of $\mu$.
...
\comment{Do we need a "Terminology" heading for this?}
\begin{terminology*}
...
\item $|\gamma(x, \smooth{\gamma} \eta(x)) \theta \int (r(y, \smooth{r} \eta(y)) - r(x, \smooth{r} \eta(x))) q_\theta(x, dy)|$
    is uniformly bounded for $x \in \IR^d$ and $\eta \in \measures$.
\end{enumerate}
...
We give two more concrete conditions under which the fourth condition is satisfied.
...
\end{enumerate} 
then
$|\gamma(x, \smooth{\gamma} \eta(x)) \theta \int (r(y, \smooth{r} \eta(y)) - r(x, \smooth{r} \eta(x))) q_\theta(x, dy)|$
is uniformly bounded for $x \in \IR^d$ and $\eta \in \measures$.
\end{lemma}
...
and the population has all individuals at a single location (so, $\eta = \delta_x$),
in which case the mean rate of change of the total population size
is $\int (r(y, N \smooth{r}(y) ) - r(x, N \smooth{r}(x))) q_\theta(x, dy)$;
the first condition of Lemma~\ref{conditions on r} would ensure this is of order $1/\theta$.
...
This will be satsified, for example, if $\rho_r$ decays 
exponentially.
...
purely atomic measures, with $\eta^N$ comprised of atoms of mass $1/N$, which is
tight 
...
\begin{corollary}
    \comment{TODO: write corollary saying what's in the previous sentence 
    and what the stationary distribution is?}
\end{corollary}
...
\comment{TODO: peter to look into this}
...
and has been described in similar models, e.g., by \citet{sasaki1997clumped,hernandezgarcia2004clustering,young2001reproductive}.
\comment{TODO: add traveling wave citations}
...
but on the other hand we expect $\gamma_0' < 0$
(i.e., per capita birth rate decreases with density near equilibrium),
and so the stability of the constant solution $\varphi \equiv \varphi_0$
...
\comment{Do we not have uniqueness, or not know if there is uniqueness?}
...
\begin{proposition}
    ...
    \comment{Theorem~\ref{thm:nonlocal_convergence}
...
\comment{In this section I haven't specified the filtration for the martingale problem.}
...
First, we prove Lemma~\ref{conditions on r}.

\begin{proof}[of Lemma~\ref{conditions on r}]
...
\comment{TODO MOVE SOME OF THIS TO PROOF OF LEMMA~\ref{lem:conditions on r}}
We need a bound on $\gamma(x,\eta)B_f(x,\eta)+F$.
First suppose that
...
this case 
    the term in~\eqref{eqn:q_bound_split} is bounded by
    \begin{align*}
       K_5
        \left|
            \theta \int_{\IR^d}
...
in the notation of Lemma~\ref{lem:eta_f_bound},
    \begin{align*}
		...
		B_1(x, \eta^N_s)-\frac{1}{N}F(x,\eta^N_s)
            \right\}, \eta_s^N(dx) \Big\rangle
            dt
            \right] \right) .
    \end{align*}
...
to the $(\Pgen^{\infty}, \eta_0)$-martingale problem if 
for all such test functions 
...
equation~(\ref{eq:boundDensityXt}), and 
...
for any convergent subsequence $\{\rho_\epsilon*\eta^N(x)dx\}_{N\geq 1}$,
\[ 
\int_0^t \big\langle \mathcal{L}^\theta T_{\epsilon^2} f, 
\eta_s^N(dx)\big\rangle ds 
