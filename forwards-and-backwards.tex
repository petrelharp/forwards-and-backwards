...
Therefore $\tau^N_{a} \sim Gamma(\sum_{i=1}^{k}a_i,1)$.
...
=& \sum_{i=1}^{k}
\sum_{j=1}^{a_i-1}
(1-\kappa_{[a]_{i-1}\concat j}) y_{[a]_{i-1}\concat j} \ind_{U^N_{[a]_{i-1}\concat j}(\tau_{[a]_{i-1}\concat j}) < N}\\
&+ \sum_{i=1}^{k}
\kappa_{[a]_i} y_{[a]_i} \ind_{U^N_{[a]_i}(\tau_{[a]_i}) < N}.
\end{aligned}    
\end{equation}
...
for each $i$ and $j$,
and that the sum above is finite,
it suffices to show that
the quantities 
\begin{equation}
\mathbb{P}(U^N_{[a]_{i-1} \concat j}(\tau_{[a]_{i-1} \concat j}) < N)    
\end{equation}
converges
for all $i=1,..,k$ and $j=1,..,a_i$.

Writing out
$U^N_{[a]_{i-1} \concat j}(\tau_{[a]_{i-1} \concat j})$,
we have
\begin{equation}
\begin{aligned}
&\mathbb{P}(U^N_{[a]_{i-1} \concat j}(\tau_{[a]_{i-1} \concat j}) < N) \\
=&\mathbb{P}(U^N_{[a]_{i-1} }(\tau_{[a]_{i-1} \concat j})
    +
    \ell(
    X^N_{[a]_{i-1} }(\tau_{[a]_{i-1} \concat j}-),
    y_{[a]_{i-1} \concat j},
    \eta^{N}(\tau_{[a]_{i-1} \concat j}-),
    v_{[a]_{i-1} \concat j})
    < N)\\
=&\mathbb{P}\left(
    \frac{
    ...
    < N-U^N_{[a]_{i-1} }(\tau_{[a]_{i-1} \concat j})
    \right)\\
=&\mathbb{P}\left(
            v^N_{[a]_{i-1} \concat j}
    < (2 \theta \gamma(x, \eta) r(x + y, \eta)-  2 N^{-1} \theta \gamma(x, \eta) r(x + y, \eta)U^N_{[a]_{i-1} }(\tau_{[a]_{i-1} \concat j}))
    \right)\\
=& \frac{(2 \theta \gamma(x, \eta) r(x + y, \eta)-  2 N^{-1} \theta \gamma(x, \eta) r(x + y, \eta)U^N_{[a]_{i-1} }(\tau_{[a]_{i-1} \concat j}))}{N}\\
\to & 2\alpha \gamma(x, \eta) r(x + y, \eta).
\end{aligned}    
\end{equation}
