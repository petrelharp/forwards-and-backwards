...
If $N=\delta\theta$, for some positive constant $\delta$,
...
$$
\frac{1}{\delta}F''(\langle f,\Xi\rangle)
...
\Xi_s(dx)\rangle ds$$
is a martingale with quadratic variation
$$\frac{2}{\delta}\int_0^t\langle f^2(x) 
...
which take values in the space of empirical measures.
On the other hand,
...
$(\eta^{\varepsilon, \theta, N}_t)_{t \geq 0}$ to $(\Xi_t)_{t \geq 0}$.
As a result, it suffices for us to focus on
tightness of $(\eta^{\theta}_t)_{t \geq 0}$
and spatial convergence of $(\eta^{\varepsilon, \theta, N}_t)_{t \geq 0}$
as $\theta \to \infty$.
...
To bound $\E\left[\langle 1, \eta_t \rangle\right]$
by Gronwall,
...
\end{align}
(since $Z \sim \text{Normal}(0, \sigma^2)$ then $\E[e^{tZ}] = \exp(\sigma^2 t^2 / 2)$).
