...
\comment{TODO: modify condition (4) to say that the $C_f$ only depends on $\|f\|_\infty$, $\|\partial f\|$ and $\|\partial^2 f\|$: PETER}
...
\begin{figure}
    ...
    \end{center}
    \caption{
        \comment{TODO PETER; maybe remove dotted lines}
...
\begin{proof}[Sketch]
    \comment{Change this to a sequence $f_n$ of smooth things with uniformly bounded derivs that increase 1; follows from condition 4. PETER}
...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
    ...
    \comment{TODO: refer to correct thing or just say "like in 6.6": PETER}
...
\comment{(TODO beware the different definition of $f$ to that lemma) PETER ``in order to adhere to standard notation in the literature''}
...
\comment{(TODO beware the different definition of $f$ to that lemma) PETER}
...
\comment{TODO; REWRITE TO SAY THAT STHG IMPLIES CONDITION 2 OF LEMMA \ref{lem:conditions on r} (see reference on p.13) PETER}
...
\begin{proof}[of Lemma \ref{lem:gamma_bound}]
    ...
    \begin{align} \label{eqn:q_bound_split2}
    &
        \left| \theta 
            \int_{\IR^n} 
                \left\{
    ...
    So,
    the second term in~\eqref{eqn:q_bound_split2} is bounded by
    \begin{align*}
...
\comment{TODO: explain what this section is doing (tightness of products) PETER}
