...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
    ...
    \comment{Should this be a separate definition (along the lines of Condition 2.1 in \citet{etheridge/kurtz:2018})?}
    Let $\{h_k\}$ be a separating class of nonnegative functions in $C^1_b([0, \infty))$
    such that
    $\int_0^\infty \int_u^\infty h_k(v) dv du$,
    $\int_0^\infty u^2 |h_k'(u)| du$,
    and $\int_0^\infty h_k(u)^2 du$
    are all finite for all $k$,
    and assume furthermore that $h_0$ is a positive, nonincreasing function 
    that bounds all other $h_k$, i.e., for any $k$,
    there is a $c_k$ such that $h_k(u) \le c_k h_0(u)$ for all $u \ge 0$.
    (For instance, we may take $h_0(u) = e^{-u}$ and the remaining $h_k$
    to be a separating class of $C^1$ functions with compact support.)
    Following \citet{etheridge/kurtz:2018} (see Condition 2.1),
    we endow $\lpmeasures$ with
    the topology induced by test functions on $\overline{\IR^d} \times [0,\infty)$
    of the form $g(x) h(u)$, where $g \in C_b(\overline{\IR^d})$ is bounded and continuous
    and $h \in \{h_k\}_{k \ge 0}$.
    This space is completely metrizable,
    and a subset $K \subset \lpmeasures$ is relatively compact if and only if
    $\sup_{\lp \in K} \langle h_k, \lp \rangle < \infty$ for each $k$;
...
\begin{lemma}
    ...
    for any $T$ and $k$ there exists a constant $K_k$ such that
    $$ 
...
\begin{proof}[of Lemma~\ref{lem:compact containment hn}]
    ...
    First note that since $h_k(u) \le c_k h_0(u)$ for all $u$,
    so $\langle h_k, \lp_t^N \rangle \le c_k \langle h_0, \lp_t^N \rangle$,
    and so we need only prove the Lemma for $h_0$.
    Henceforth, let $h = h_0$.
    ...
    and we have assumed that $h'(u) \le 0$ (since $h_0$ is nonincreasing),
    so we may bound
    ...
    by our assumption that $\int_0^\infty h_0(v)^2 dv < \infty$.
...
\begin{proof}[of Lemma \ref{lem:test_fn_tightness}]
    ...
    Lemma~\ref{lem:eta_projections_tightness},
    except that terms are bounded by constant multiples of $\langle h_0, \xi^N_s \rangle$
