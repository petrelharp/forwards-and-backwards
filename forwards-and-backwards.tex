...
of Section~\ref{sec: individual lines of descent} holds.
    ...
    \comment{PETER: maybe? Figuring out how to make it more concrete/rigorous seems daunting, though -
...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
    ...
    % PLR: NEW DETAILS
    Several places below we require a dense subset
    ...
    % END NEW DETAILS
    Topologized in this way, the space $\lpmeasures$ completely metrizable,
    ...
    For \citet{ethier/kurtz:1986} Theorem~3.9.1
    ...
    % % (i.e., are less than $\epsilon$ outside of a compact set, for any $\epsilon$).
    % % Since $C_0(\lpmeasures)$ is dense
    % in $\bar C(\lpmeasures)$ (with respect to uniform convergence on compact subsets).  So, 
    Since as above the algebra generated by these are dense in $\bar C(\lpmeasures)$,
...
is relatively compact by appropriate choice of $\{f_k\}_{k \ge 1}$.
% \comment{
...
\begin{proof}[of Lemma~\ref{lem:h_containment}]
    ...
    we know that if $f \in C(\IR^d \times [0,\infty))$
    with $\IE[\langle |f|, \lp^N_t \rangle] < \infty$ then
    ...
    Since $\sqrt{x} \le 1 + x$ for $x \ge 0$,
...
at location $x$ and time $t$ (which may only exist almost everywhere).
    ...
    \comment{PETER: I think the point here was to remind the reader that it may not exist everywhere?}
...
$\partial_t \varphi_t = r \DG^*[ \varphi_t \gamma ] + \varphi F$.
