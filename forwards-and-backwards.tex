...
\begin{proof}
    \comment{TODO: copy over from below}
\end{proof}
...
\begin{proof}
    We first take $f \equiv 1$ in Equation \eqref{eqn:eta_martingale},
    so that
    \begin{align} \label{eqn:mass_martingale_decomp}
        \langle 1, \eta^N_t \rangle
        &=
        \langle 1, \eta^N_0 \rangle
        ...
        + M^N_t(1) ,
    \end{align}
    where $M^N_t(1)$ is a martingale and for the moment we are defining
    \begin{equation*}
        B(x, \eta) = \gamma(x, \eta) \int_{\IR^d} \theta (r(y, \eta) - r(x, \eta)) q_\theta(x, dy) .
    \end{equation*}
    By Lemma~\ref{lem:eta_compact_containment} and the assumption that $F$ is bounded above,
    $B(x, \eta) + F(x, \eta) \le C$ for some $C$,
    and so, taking expectations,
    \begin{align*}
        \IE\left[ \langle 1, \eta^N_t \rangle \right]
        &\le
        \IE\left[ \langle 1, \eta^N_0 \rangle \right]
        + C \int_0^t \IE\left[ \langle 1, \eta^N_s \rangle \right] dt .
    \end{align*}
    Applying Grownwall's inequality, we have that
    \begin{align*}
        \IE\left[ \langle 1, \eta^N_t \rangle \right]
        \le
        \IE\left[ \langle 1, \eta^N_0 \rangle \right] e^{Ct} .
    \end{align*}
    Now, let $M^*_t(\phi) = \sup_{0 \le s \le t} M_t(\phi)$,
    and as before $[M(\phi)]_t$ the quadratic variation of $M_t(\phi)$,
    ...
    \end{align*}
    Taking suprema and expectations on both sides of equation~\eqref{eqn:mass_martingale_decomp},
    then again using the fact that $B(x, \eta) + F(x, \eta) \le C$,
    \begin{align*}
...
\begin{proof}
As in the proof of Lemma~\ref{lem:gamma_bound},
we use the semimartingale representation of $\langle f, \eta^N_t\rangle$
from Definition~\ref{defn:mgale_construction},
\begin{align*}
    \langle f, \eta^{N}_{\tau+s}\rangle
    -
    \langle f, \eta^{N}_{\tau}\rangle
    &=
    M^N_{\tau + s}(f) - M^N_\tau(f)
    +
    \int_\tau^{\tau + s} 
    \int_{\IR^d}
    \left\{
...
$$
    B_f(x, \eta)
    =
    \gamma(x, \eta) \int_{\IR^d} \theta ( f(y) r(y, \eta) - f(x) r(x, \eta) ) q_\theta(x, dy) .
$$
% By Lemma~\ref{lem:gamma_bound} and the assumption that $F$ is bounded above,
% $B_f(x, \eta) + f(x) F(x, \eta) \le C_f$ for some constant $C_f$.
We apply the Aldous-Rebolledo criterion \citep{rebolledo:1980}, which says that
...
\comment{The following is NOT RIGHT because $F$ is not bounded below!}
...
\end{multline}
since the first term is bounded above by $C_{\gamma,r}f(x)$ by Lemma \ref{cor:gamma_bound}
...
Therefore, by Lemma \ref{lem:eta_compact_containment},
...
which is proven in
Theorem \ref{lem:eta_projections_tightness}.
\end{proof}
...
Note: $F$ no longer assumed polynomial.
}
