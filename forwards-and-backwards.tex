...
If either
    \comment{which assumptions from 2.7 are in force? since in (1) we need $\gamma$ to be bounded, which is condition 3 above}
\begin{enumerate}
\item
...
\comment{clarify why: smaller $N_e$}
...
\comment{this section (6) is no long just "proofs" since they're in section 7 and 8 too}
\comment{make sure this reads okay and describes just this section (6); the end-of-the-intro paragraph describes structure}
...
In Section~\ref{subsec:one step convergence proof}
...
Finally, in Section~\ref{sec:lookdown_proofs} we prove our results concerning both convergence of the lookdown representation
...
\comment{perhaps $B$ should have a superscript $\theta$}
\begin{equation}
\label{generator Bf}
...
\begin{align} \label{eqn:eta_f_mgale_decomp}
    ...
    + M^N_t(f) ,
\end{align}
where $M^N_t(f)$ is a martingale and $B_f$ is defined in \ref{generator Bf}.
...
\label{exponential moment}
    ...
    \eta_s(dx)\Big\rangle ds\Big] .
\end{multline} 
    \comment{alison to look at this again; check whether it depends on just bounding $B_1$}
...
\comment{below $\eta_s^N$ stops having  an $N$ at some point}
...
\comment{explain that's where we use bounded-below-ness of $\theta \epsilon^2$.}
...
\comment{ACTUALLY BETTER we want to transfer $T_\epsilon^2$ over onto $\eta$; this happens in two steps, first this proposition; next using that $T_{\epsilon^2}f - f$ is small}
...
=0,
\end{equation}
\comment{which we prove by showing the expectation of this positive quantity goes to zero; e.g. see Lemma 7.5(?)}.
...
\comment{key bit is that we can always write $M_s(w)$ as a convolution of $p_{\epsilon^2/}$ with something}
...
\comment{TODO; REWRITE TO SAY THAT STHG IMPLIES CONDITION 2 OF LEMMA \ref{lem:conditions on r} (see reference on p.13)}
...
\begin{proof}[of Lemma \ref{lem:gamma_bound}]
    ...
    \begin{align*}
        % note that the f' term does not depend on x, just y,
        % so can come out of the integral
        \|f'\|_\infty
        \left|
            \theta \int_{\IR^d}
                (\smooth{r}\eta(y) - \smooth{r}\eta(x))
