...
(visible in equation~\eqref{eq: Pre-limit Martingale Characterisation}
...
% of equations~\eqref{eq: Pre-limit Martingale Characterisation} and \eqref{eqn:prelimit_martingale_variation}
...
of the total net reproduction rate.
This does not follow immediately from our assumption that $F$ is bounded above
because reproduction is not local.
\comment{say more about the difference between $F$ and the total net reproduction rate}
...
\begin{lemma}
    ...
    then there exists a $C_f < \infty$ such that
...
\begin{proof}[of Lemma \ref{lem:q_bound}]
    The key assumption here from Definition~\ref{def:model_setup}
    will be that $\epsilon_{r}^2 + \frac{2\lambda}{\theta} < \epsilon_{\gamma}^2$,
    ...
    so that $\int p_{\epsilon_r^2}(y - w) q_\theta(x, dy) = p_{\epsilon_r^2, 1/\theta, x}(w-x)$.
    ...
    Since $\partial_s p_{s, x}(z) / p_{s, x}(z)$ is a polynomial in $z$
    with coefficients that are uniformly bounded for $s \ge 1$,
    it is evident that
    $\sup_z \partial_s p_{\epsilon_r^2, s, x}(z) / p_{\epsilon_\gamma^2}(z) < \infty$
    if and only if
    \begin{align*}
        \frac{1}{2\epsilon_\gamma^2} \|z\|^2
        -\frac{1}{2} (z - s\meanq(x))^T \Gamma(s,x)^{-1} (z - s\meanq(x))
    \end{align*}
    is negative for all $z$ outside a bounded region.
    ...
    where $Y_1$ and $Y_2$ have covariance $\sigma^2_r I$,
    ...
    As before, $\partial_s p_{s, t, x}(y_1, y_2)$ has polynomial terms multiplying
    $$
        \exp\left(
            - \frac{1}{2} [y_1, y_2[^T \Gamma(s, t, x)^{-1} [y_1, y_2]
        \right) ,
    $$
    where $[y_1, y_2]$ is the $\IR^{2d}$ vector formed by concatenating $y_1$ and $y_2$,
    ...
    \begin{align*}
        &
        (\|y_1\|^2 + \|y_2\|^2) / \epsilon^2_\gamma
            - [y_1, y_2[^T \Gamma(s, t, x)^{-1} [y_1, y_2]
        \\ &\qquad \le
        (\|y_1\|^2 + \|y_2\|^2) \left(
...
\begin{corollary}
    \label{cor:gamma_bound}
    Under the assumptions of Definition~\ref{def:model_setup},
    including that $\epsilon_{r}^2 + \frac{1}{\theta} < \epsilon_{\gamma}^2$,
    for all $f \in \mathcal{C}^{2}_{0}(\IR^d \times \IR)$
    with bounded first and second derivatives
    and for all $\eta \in \measures$, we have
...
\comment{This is not enough, since we need to show the constant holds for all $x$, and $J$ depends on $x$.}
...
\end{aligned}
\end{equation}
where the last inequality holds by Lemma \ref{cor:gamma_bound}.
