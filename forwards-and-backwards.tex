...
the level $u$ of an individual at $x$
...
(This differs from $F = r\gamma - \mu$ defined above
because $b$ contains the adjustment to fecundity obtained by
averaging over establishment probabilities in the locations that offspring disperse to.)
We can then rewrite the differential equation 
governing the how each individual's level evolves as
\begin{align}
\dot{u}
    &=
    \lambda \gamma(x,\eta) \int_{\IR^d} r(y, \eta) q(x, dy)
    \left\{
        \left(1-\frac{u}{\lambda}\right)^{2}
        - 1
        + 2 \frac{u}{\lambda}
    \right\}
    -
...
\comment{PLR edited through to here.}


The generator of $\lp$ applied to functions of this form
is therefore
\begin{eqnarray}
&&
A f(\lp ) \nonumber \\
&&\quad
    = f(\lp) \,
    \sum_{(x,u) \in \lp}\,
        2 \lambda^{-1} \gamma(x, \eta)
    \int_u^\lambda \Bigg(
        \frac{1}{2} \frac{
            \int_{\mathbb{R}^d} g(y,u) r(y,\eta) q(x, dy) g(x,v_1)
        }{
            g(x,u)
        }
\label{multgen3}\\
&&\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad {}
        + \frac{1}{2} \int_{\mathbb{R}^d} g(y,v_1) r(y,\eta) q(x,dy)
        - 1
    \Bigg) dv_1
\nonumber\\
&&\quad \quad
...
&&\quad
    = f(\lp) \,
    \sum_{(x,u)\in\lp}\,
    2 \gamma(x, \eta) \frac{\theta}{\lambda}
    \int_u^\lambda \Bigg(
        \frac{1}{2} \frac{
            \int_{\mathbb{R}^d}( g(y,u) - g(x,u) ) r(y, \eta) q(x,dy) g(x,v_1)
        }{
            g(x,u)
        }
\non\\
...
to either the parent or child being reassigned the new level $v_1$
during a birth event.
In the first case, the parent at location $x$,
which had level $u$ before the event, is reassigned to level $v_1$,
and the child is inserted at level $u$ and location $y$;
...
\comment{TODO: this is a bit redundant with the above.
Instead, maybe (a) describe the process in words; (b) give this definition;
then (c) explain why it's the generator, so we only write down the generator once?}
...
\comment{Remark on the Remark: this is more than one remark.
I'm not sure it needs to be in a Remark block: how about just a few paragraphs?}
...
and for any $\xi \in \mathcal{N}_f(\mathbb{R}^d \times [0,\infty))$, we denote
$$\hat{f}(\xi)=\int f(\xi) \Gamma (d\textbf{u})$$ 
to be the spatial test functions with averaged level,
then the process
\begin{equation}
\begin{aligned}
M^{\theta}_t:=&\hat{f}(\xi_t)-\hat{f}(\xi_0)-\int_{0}^{t}\int   A^{\theta}f(\xi_s)\Gamma(d\textbf{u})ds\\
:=&\hat{f}(\xi_t)-\hat{f}(\xi_0)-\int_{0}^{t}   \Pgen^{\theta}\hat{f}(\xi_s)ds
\end{aligned}    
\end{equation}
is a martingale.
...
Therefore, we use the same notation $\eta$
in our spatial model
to denote the spatial projection of $\xi$,
and we reserve the notation $\xi$
