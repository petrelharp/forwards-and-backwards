...
\begin{theorem} \label{TeoremOneStepConvergence}
    ...
    such that
    \begin{enumerate}
        \item $\langle 1 , \eta_0 \rangle < C $
        \item  $\| \rho_\epsilon * \eta_0(x) \|_\infty < C$ for all $x \in \IR^d$
        \item $|\rho_\epsilon * \eta_0(x) - \rho_\epsilon * \eta_0(y)| \leq C \| x - y \|$ for all $x,y \in \IR^d$, and
        \item $\rho_\epsilon * \eta_0(x) dx$ converges weakly to $u_0(x)dx$ for some $u_0(x) \in \mathcal{C}^2(\IR^d)$.
    \end{enumerate}
...
\begin{proof}[of Proposition~\ref{Prop:SecondMomentMass}]
    % Referring to section~\ref{sec:heuristics} with $f=1$ and $G(m) = m^2$,
