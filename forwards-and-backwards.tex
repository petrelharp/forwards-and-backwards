...
with label $i$, location $X_i(0) = x_i$, level $U_i(0) = u_i$, and birth time $\tau_i = 0$.
...
\begin{lemma}
The lookdown representation satisfies
Equation \eqref{eq: vague compact containment condition}, i.e.
...
\begin{proof}
For every $f \in C^{+}_{c}(\overline{\mathbb{R}^d} \times [0,\infty))$,
there exists $u^* > 0$ such that $f(x,u) = 0$ for all $u \geq u^*$.
Then,
\begin{equation}
\begin{aligned}
 &\limsup_{n \to \infty} \mathbb{P}\left\{ \sup_{t\in [0,T]}\langle f, \xi^n_t \rangle > N \right\} \\
 \leq & \limsup_{n \to \infty}\frac{\mathbb{E}\left\{ \sup_{t\in [0,T]}\langle f, \xi^n_t \rangle \right\}}{N}\\
 \leq & \limsup_{n \to \infty}||f||_{\infty}\frac{\mathbb{E}\left\{ \sup_{t\in [0,T]}\langle 1_{[0,u^*]}, \xi^n_t \rangle \right\}}{N}.
\end{aligned}
\end{equation}
...
Therefore, 
\begin{equation}
\mathbb{E}\left\{ \sup_{t\in [0,T]}\langle 1_{[0,u^*]}, \xi^n_t \rangle \right\} = u^* \mathbb{E}\left\{ \sup_{t\in [0,T]}\langle 1, \eta^n_t \rangle \right\}.
\end{equation}
Finally, 
recall that the supremum of the empirical measure process is controlled in Lemma \ref{lem:eta_compact_containment}
so there exists some constant $C_T$ such that 
\begin{equation}
\begin{aligned}
 &\limsup_{n \to \infty} \mathbb{P}\left\{ \sup_{t\in [0,T]}\langle f, \xi^n_t \rangle > N \right\} \\
 \leq & \limsup_{n \to \infty}||f||_{\infty}\frac{u^*}{N}\mathbb{E}\left\{ \sup_{t\in [0,T]}\langle 1, \eta^n_t \rangle \right\}\\
 \leq & ||f||_{\infty}\frac{u^*}{N}C_T\\
 < & \epsilon
\end{aligned}
\end{equation}
...
To characterise convergence of $\xi^N(t)$ as $N \to \infty$,
we therefore need to impose an appropriate topology on $\mathcal{M}(\overline{\mathbb{R}^d} \times [0,\infty))$,
the space of finite measures on $\overline{\mathbb{R}^d} \times [0,\infty)$.

We now refer to the vague topology introduced in P564, \S A2 in \cite{kallenberg1997foundations}.
...
We now introduce two important theorems on compactness and convergence in the vague topology.
\begin{theorem}[Theorem A.23 in \cite{kallenberg1997foundations}]
For any locally compact, second-countable Hausdorff Space $S$, 
we have
\begin{itemize}
\item $\mathcal{S}$ is Polish in the vague topology,
\item a set $A\subset \mathcal{S}$ is vaguely relatively compact iff $\sup_{\mu\in A} \mu f < \infty$ for all $f \in C^{+}_{c}$
...
\begin{corollary}
The set $\Gamma_K := \{\mu \in \mathcal{M}(S): \mu(S) \leq K\}$
is vaguely compact in $\mathcal{M}(S)$.
\end{corollary}
\begin{proof}
It suffices to show that $\Gamma_K$ is vaguely relatively compact and vaguely closed.
For any fixed $f \in C^{+}_c$,
we have that $ \sup_{\mu \in A} \mu f \leq  ||f||_{\infty} \mu(S)  \leq ||f|| K < \infty$,
so $\Gamma_K$ is vaguely relatively compact.

Now it suffices to prove that $\Gamma_K$ is closed under the vague topology.
Consider a vaguely converging sequence $(\mu_n)_{n \geq 1}$ in $\Gamma_K$
satisfying $\mu_n \to \mu \in \mathcal{M}(S)$,
then for an increasing sequence of functions $f_m \to 1$,
by the Dominating Convergence Theorem, 
\begin{equation}
\mu(S)
= \lim_{m \to \infty} \mu f_m
= \lim_{m \to \infty} \lim_{n \to \infty} \mu_n f_m
= \lim_{n \to \infty}   \mu_n (S)
\leq K,
\end{equation}
and so $\mu \in \Gamma_K$.
Therefore $\Gamma_K$ is vaguely compact. 
\end{proof}

We now include another theorem that is very relevant to establishing tightness of $\mathcal{M}(S)$-valued processes in the Skorokhod topology.

\begin{theorem}[Theorem A2.4 in \cite{kallenberg1997foundations}]\label{teo: vag_sko_conv}
Let $f_1,f_2,...$ be a dense collection in $C^{+}_{c}(S)$ closed under addition, 	
then any set $A \subset \mathcal{D}_{\mathcal{M}(S)}([0,\infty))$
is relatively compact if and only if for every $j \in \mathbb{N}$,
$Af_j = \{xf_j, x \in A\}$
is relatively compact in $\mathcal{D}_{\mathbb{R}^{+}}([0,\infty))$.
\end{theorem}

To characterise convergence of lookdown representations,
we take $S = \overline{\mathbb{R}^d} \times [0,\infty)$
and consider the test functions of the form 
\begin{equation}
...
Combining the above observation and Theorem \ref{teo: vag_sko_conv},
...
Furthermore, assume that for any $f \in C^{+}_{c}(\overline{\mathbb{R}^d} \times [0,\infty))$ and $\epsilon > 0$, there exists $N>0$ (dependent on $f$ and $\epsilon$), 
such that 
\begin{equation}
...
\end{equation}
 ...
 then $(\xi^n_t)_{t\geq 0}$ is relatively compact in $\mathcal{D}_{\overline{\mathbb{R}^d} \times[0,\infty)}([0,T])$.
\end{lemma}

\begin{proof}
By Theorem \ref{teo: vag_sko_conv},
it suffices to prove that for all $f \in C^{+}_{c}(\overline{\mathbb{R}^d} \times [0,\infty))$,
$\{(f(\xi^n_t))_{t \geq 0}: n=1,2,.. \}$ is relatively compact.

We now make use of the Aldous Criterion of tightness on the real-valued sequences
$\{(f(\xi^n_t))_{t \geq 0}: n=1,2,.. \}$. It suffices to show that
...
\end{equation}
 ...
 We have proved relative compactness of $\{(f(\xi^n_t))_{t \geq 0}: n=1,2,.. \}$ and thus relative compactness of  $(\xi^n_t)_{t\geq 0}$ in $\mathcal{D}_{\overline{\mathbb{R}^d} \times[0,\infty)}([0,T])$.
\end{proof}
