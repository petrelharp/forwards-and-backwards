...
\comment{TODO: cite \citet{lam2023introduction,cantrell2004spatial}}
...
\begin{corollary} \label{cor:stationary_dist}
    ...
    the population process is stationary (so $\varphi_t \equiv \varphi$),
...
\begin{align} \label{eqn:conditionally_uniform}
    ...
    &=
    e^{-\langle F_f^N, \kappa(\lp) \rangle} ,
\end{align}
where
...
\begin{proposition}
    ...
    such that $\eta^N = \kappa \circ \lp^N$
...
\begin{theorem}
    ...
    suppose also that $\eta^N_0 \to \eta_0$ (as finite measures on $\IR^d$),
...
\begin{proof}[of Proposition~\ref{prop:limiting_construction}]
    ...
    Theorem~\ref{thm:mmt_application} shows that
...
\begin{equation}
\label{generator Bf}
    B_f(x, \eta) = \theta \int_{\IR^d} (f(y) r(y, \eta) - f(x) r(x, \eta)) q_\theta(x, dy) .
\end{equation}

First, we prove Lemma~\ref{lem:conditions on r}.

\begin{proof}[of Lemma~\ref{lem:conditions on r}]
Here, we need to prove that $|\gamma(x, \eta) B_1(x, \eta)|$ is bounded,
...
We write
\begin{align*}
    r\big(y,\rho_r*\eta(y)\big) - r\big(x,\rho_r*\eta(x)\big)
    &=
    \sum_i (y_i - x_i) \partial_{x_i} \Big(r\big(x,\rho_r*\eta(x)\big)\Big) 
    \\ &\qquad {}
    +
    \frac{1}{2} \sum_{ij} (y_i - x_i)(y_j - x_j)
        \partial_{x_i x_j} \Big(r\big(\smooth{r}\eta(z)\big)\Big) ,
\end{align*}
...
in this expression and so we have a bound of the form
\begin{multline*}
\Big| r\big(y,\rho_r*\eta(y)\big) - r\big(x,\rho_r*\eta(x)\big) \Big|
\\
\leq
        K_1 \left|
...
u implies that $|B_1|$ is uniformly bounded;
since $\gamma$ is bounded as well, so is $|\gamma B_1|$.
...
\left| \theta 
    \int_{\IR^n} 
        \big\{
            r\big(x,\smooth{r}\eta(y)\big)
            -
            r\big(x,\smooth{r}\eta(x)\big)
        \big\}
    q_{\theta}(x,dy) 
\right| 
\\ &\qquad
...
\end{align*}
Therefore,
under Condition~\ref{control through gamma}, $|B_1(x,\eta)|$ is bounded by 
a constant times $\smooth{\gamma}\eta(x)+(\smooth{\gamma}\eta(x))^2$;
since $m^2\gamma(m)$ is bounded, so is $|\gamma(x,\eta) B_1(x,\eta)$.
\end{proof}
...
tightness of the population processes in $\mathcal{D}_{\cmeasures}([0,\infty))$
follows from tightness of the real-valued processes $(G(\eta_t))_{t\geq 0}$ for a sufficiently large
class of test functions $G$, which we establish through an application of the
...
We need a bound on $\gamma(x,\eta)B_f(x,\eta)+fF$.
First note that we may rewrite $B_f(x, \eta)$ as
\begin{align}
    \nonumber
&
    \left| \theta 
...
\\ 
    ...
    \|f\|_\infty |B_1(x, \eta)| .
\end{align}
The first term is bounded uniformly
because $q_\theta(x, dy)$ is the density of a Gaussian with mean $\meanq(x)/\theta$
and covariance $\covq(x)/\theta$,
and both $\meanq(x)$ and $\covq(x)$
are bounded uniformly in $x$ under Assumptions~\ref{def:model_setup}.
Furthermore, Condition~\ref{gamma_B_condition} of Assumptions~\ref{def:model_setup}
stipulates that $|\gamma(x, \eta) B_1(x, \eta)|$ is uniformly bounded,
...
of the martingale problem for the limiting process,
that follows by application of It\^o's lemma.
...
is a solution to the $(A, \lp_0)$-martingale problem of Definition~\ref{defn:limiting_lookdown_mgale}
if and only if it is a solution 
to the $(\Pgen^{\infty}, \eta_0)$-martingale problem, i.e.,
if for all such test functions 
$$
    M_t :=
    G_f(\eta^{\infty}_t)-G_f(\eta_0)
    - \int_{0}^{t}\Pgen^{\infty}G_f(\eta^{\infty}_s)ds
$$
is a martingale (with respect to the natural $\sigma$-field). 
\end{lemma}

\comment{TODO: define $\mathcal{P}^N$.}
...
\begin{lemma} \label{PsiBoundHS}
    ...
    \comment{Suppose also that $\theta \epsilon^2$ is bounded below.
    (We could alternatively put a $1/\theta\epsilon^2$ term in the lemma.)}
    Then
    \[
        \psi^{\epsilon,x}_t(y)
        =
...
and so this quantity is bounded uniformly over only $t$ but also $\epsilon$.
\comment{That's where we use bounded-below-ness of $\theta \epsilon^2$.}
...
\begin{proof}[of Proposition~\ref{thm:mmt_application}]
    ...
    Theorem A.2 in \citet{etheridge/kurtz:2019},
...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
    ...
    therefore, compact containment will follow from Lemma~\ref{lem:compact containment lookdown}.
    ...
    \comment{TODO: refer to correct thing}
...
\begin{lemma}
    ...
    for any $T$ there exists a constant $K(T)$ such that for all $M > 0$,
    $$ 
...
\begin{lemma}[Compact containment for $\lp$] \label{lem:compact containment lookdown}
    ...
    in the sense of \eqref{eqn:conditionally_poisson},
    then for any $T$ and $\delta > 0$ there exists a sequence
...
\begin{proof}[of Lemma \ref{lem:test_fn_tightness}]
    ...
    \begin{align*}
        ...
        + M^f_t
        + \int_0^t \Big\langle
            c_\theta \int_u^N \int (f(y, u_1) + f(x, u_1)) q^m(x, dy) du_1
        \\ {} &\qquad
            + c_\theta (N - u) \int (f(y, u) - f(x, u)) q^m(x, dy)
...
$$
Since this is symmetric, the process $Y$ is reversible with respect to $\pi$;
