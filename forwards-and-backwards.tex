...
\comment{TODO: talk about $N \to \infty$ rather than $N \to \infty$.}
...
\begin{definition}[Model parameterization and assumptions]
    ...
    with rate $\theta \gamma(x, \smooth{\gamma} \eta(x)) N \eta_s(dx)$
    and adults die at location $x$ with rate $\theta \mu(x, \smooth{\mu} \eta(x)) N \eta_s(dx)$,
...
$$
and similiarly write $\mu(x, \eta)$  for $\mu(x, \smooth{\mu} \eta(x))$ and $r(x, \eta)$ for $r(x, \smooth{r} \eta(x))$.
When convenient, we may also suppress the arguments completely,
writing simply $\gamma$, $\mu$, and $r$ for these quantities.

\comment{I'm not quite sure how we want to frame these two things: probably not as two definitions...}
We will now give another, equivalent definition of $(\eta^{\theta}(t))_{t \geq 0}$,
this time as a solution to a martingale problem.
...
depended on $\theta$, but converged as $N \to \infty$.
...
(\gamma(x,\Xi_s)r(x,\Xi_s)+\mu(x,\Xi_s)),\Xi_s(dx)\rangle ds.$$
...
it is not too hard to show that as $N \to \infty$,
\begin{align*}
...
Definition \ref{Def: Rescaled Models},
