...
\comment{TODO: ALISON SAYS
The proof of Lemma 6.7 is now problematic. We'll be hard pushed to find a sequence of functions increasing to x, or indeed -x. Easily fixed of course, but I think that we should anyway change the condition of the theorem to be a bit more general, so say that we assume that $\langle f(x),\eta^N_0(dx)\rangle$  is uniformly bounded for some non-negative, twice differentiable with uniformly bounded first and second derivatives, function $f$ with $f(x)\to\infty$ as $x\to\infty$ and then use Lemma 6.1 and Gronwall to deduce $\langle f,\eta_t\rangle$ has bounded mean for any limit point and so Markov's inequality tells us no mass escaped to infinity.
}

\comment{``reaction diffusion equation'' or ``reaction-diffusion equation''?}
...
of~\citet{demasi/ferrari/lebowitz:1986, demasi/presutti:1991, oeschlaeger:1985},
...
The Appendix contains a few more techinical and less central lemmas.
...
\comment{TODO: restrict to Gaussian case}
\begin{proposition}
...
combined with Proposition \ref{prop:nonlocal_to_local} or~\label{nonlocalPME to PME}
