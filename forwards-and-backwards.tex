...
\comment{TODO: revisit this paragraph}
...
Here, we need to prove that $|\gamma(x, \eta) B^\theta_1(x, \eta)|$ is bounded,
...
\comment{typo}
u implies that $|B^\theta_1|$ is uniformly bounded;
since $\gamma$ is bounded as well, so is $|\gamma B^\theta_1|$.
...
\end{align*}
Therefore,
under Condition~\ref{control through gamma}, $|B^\theta_1(x,\eta)|$ is bounded by 
a constant times $\smooth{\gamma}\eta(x)+(\smooth{\gamma}\eta(x))^2$;
since $m^2\gamma(m)$ is bounded, so is $|\gamma(x,\eta) B^\theta_1(x,\eta)|$.
\end{proof}
...
We need a bound on $\gamma(x,\eta)B^\theta_f(x,\eta)+fF$.
First note that we may rewrite $B^\theta_f(x, \eta)$ as
\begin{align}
    \nonumber
&
    \left| \theta 
...
stipulates that $|\gamma(x, \eta) B^\theta_1(x, \eta)|$ is uniformly bounded,
...
\label{exponential moment}
    ...
    \eta_s(dx)\Big\rangle ds\Big] .
\end{multline} 
    \comment{alison to look at this again; check whether it depends on just bounding $B^\theta_1$}
...
\comment{TODO copy in the MMT and refer to it earlier}
