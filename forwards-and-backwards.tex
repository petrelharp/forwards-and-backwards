...
$\rho_\gamma = p_{\epsilon^2_\gamma}$,
and that %$\psi_0^\epsilon\in L^1(\IR^d)$, 
...
(which recall for this Proposition we suppose is Gaussian
and equal to $p_{\epsilon^2_\gamma}$).
...
\right) .
\end{equation*}
% and we assume that $\rho=\zeta*\check{\zeta}$ with $\zeta$ a rapidly decreasing function. 
% The example we have in mind is $\zeta$ (and therefore $\rho$) being
