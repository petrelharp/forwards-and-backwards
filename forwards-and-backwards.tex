...
\comment{TODO: signpost where proofs can be found}
...
the quadratic variation of $\widehat{M}^N(f)$ satisfies
$ \IE[[\widehat{M}^N_f]_T] \le C (\theta/N) \int_0^T \langle 1, \eta_s \rangle ds \le C' \theta/N$
for some constants $C$ and $C'$.
Now, by the Burkholder-Davis-Gundy inequality,
...
\label{martingale term to zero}
    ...
    \Big]
    \leq
    \limsup_{N\to\infty} \frac{36}{\nu^2} C'' \IE\big[[\widehat{M}^N(f)]_T\big]
    \leq
...
\nonumber
    \\ &\qquad \qquad {}
	+ \big\langle\frac{C}{(\theta\epsilon^2)^{1/2}} \Big(%\big\langle 
	p_{6(\epsilon^2+s)}(y,z)+p_{6(\epsilon^2+s)}(w,z)\Big) %, \eta_0(dz)
%\big\rangle
	+\widehat{\epsilon}(\delta,\epsilon,\theta) %\langle 1
	,\eta_0(dz)\rangle
	\nonumber
    \\ &\qquad \qquad {}
    +
...
first establishing the basic connection between the processes, 
\comment{which processes?}
...
\begin{proof}[of Proposition~\ref{thm:mmt_application}]
	...
	\comment{technically need $\psi\geq 1$, so usually just add $1$ to 
	whatever you think $\psi$ should be.} 
	...
	\comment{should we rephrase to say that the most troublesome term is...
	(because the other one is as above)?}
	...
	\comment{again add one - and inside the integral to deal with
	the non-troublesome term}
...
follows a similar pattern to those \comment{`that'?} of convergence for the population processes
...
of Section~\ref{sec: individual lines of descent} holds.
	\comment{Is this paragraph really a separate remark that we want to make?}
...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
	...
	\comment{characterisation of limit points (given that we have
	no uniqueness of the limit)?}
	...
	\comment{Do we have a reference for this claim about completely
	metrizable and these sets being enough to 
	establish relative compactness? The reader may reasonably
	ask why we had to add a point at infinity to $\IR^d$ and not one
	to $[0,\infty)$. Also, maybe spell out that the support of $f_k$ is compact in
	the product space of $\overline{\IR^d}$ times levels?} 
	...
	\comment{I don't understand vanish nowhere - I think it may because
	of the way I am reading the sentence, but it is perfectly possible
	for $\langle f,\xi\rangle$ to be zero.}
	so
...
We postpone the proof of this Lemma until we have shown how it yields 
\comment{delete `to'} to compact containment.
First, we show that this implies compact containment
\comment{compact containment of what?} for arbitrary
...
\begin{lemma}
	...
	\comment{sounds like $f$ vanishes for all $x$ irrespective of $u$!
	Can we reword? `all $(x,u)$ with $u\geq u_f$' or something?}
...
\comment{Does the reader know how to act `appropriately' in 
making this choice? Maybe refer back to the right bit above where it 
is explained.}
% \comment{
...
\begin{proof}[of Lemma~\ref{lem:h_containment}]
    ...
    we know that for appropriate $f$, \comment{A bit vague... can we be 
    more specific about what makes a function appropriate?}
    ...
    Again \comment{Only a very alert reader will remember why we say `again'
    here...} since $\sqrt{x} \le 1 + x$ for $x \ge 0$,
...
\begin{lemma}
	...
	are \comment{`is' - the subject of the sentence is the (single) sequence}
	tight in the space of \comment{real-valued? or ${\cal D}_\IR$-valued} 
	c\`adl\`ag processes.
\end{lemma}
...
\begin{proof}[of Lemma \ref{lem:test_fn_tightness}]
    ...
    \begin{align*}
        ...
        \Big\rangle ds
    \end{align*}
	\comment{limits of integration missing in last integral}
...
at location $x$ and time $t$ (which may only exist almost everywhere).
	\comment{Is it obvious that it exists almost everywhere? 
	We just need to be able to integrate against it I guess.}
...
The diffusion is time-inhomogeneous if the density is not constant with
	\comment{this may be an example of being divided by a common
	language, but I would say `in time'} time.
Let $\varphi_t(x)$ be the limiting density, which 
	\comment{is a weak solution to?} 
	solves \eqref{general deterministic limit},
...
\comment{This is a bit confusing. I think it is partly because it says generator
of $\widehat{T}_{t,s}$ and of course $t$ disappears. But also it is 
because we are 
looking for the generator of the ancestral lineage and so it 
feels like one wants to 
differentiate with respect to `-s' if you see what I mean.}
and writing 
...
First, note that in this case
	\comment{Should really define our notation - ${\cal L}_s$ is
