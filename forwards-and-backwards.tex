...
\comment{FROM ALISON: I rewrote the rest of this paragraph. Missing references
are in the latex file, next to the citation, but commented out}
Following the now classical contributions 
of~\citet{demasi/ferrari/lebowitz:1986, demasi/presutti:1991,
oeschlaeger:1985},
...
%}
% A. De Masi and E. Presutti. Mathematical methods for hydrodynamic limits,
%volume 1501 of Lecture Notes in Mathematics. Springer-Verlag, Berlin, 1991.
%
% K. Oeschl\"ager. A law of large numbers for moderately interacting 
% diffusion processes. Z. Warsch. Verw. Gebiete. 69(2):279-322, 1985
...
\citet{oelschlaeger:1989, flandoli/leimbach/olivera:2019,
flandoli/huang:2021}, 
% K. Oeschl\"ager. On the derivation of reaction-diffusion equations as limits
% of systems of moderately interacting stochastic processes. Probab. Th. Rel.
% Fields 82, 565--586, (1989)
%
% F. Flandoli, M. Leimbach, and C. Olivera. Uniform convergence of 
%proliferating particles to the FKPP equation. J. Math. Anal. Appl. 473(1):
%27--52, (2019)
...
\comment{The missing reference is
 G. Birzu, O. Hallatshcek and K. Korolev. Genealogical structure
 changes as range expansions transition from pushed to pulled. 
 PNAS 118(34) (2021) 
 https://doi.org/10.1073/pnas.2026746118}
...
We now give two concrete situations in which the fourth condition is satisfied.
...
\begin{theorem} \label{thm:nonlocal_convergence}
    ...
    which is tight in $\measures$ and such that
    $\langle |x|, \eta_0^N(dx)\rangle<C<\infty$
...
\begin{figure}
    ...
    \caption{
        ...
        values of $\theta$ are 1 and 100, respectively, and both values of $N$ are the same.
	\comment{FROM ALISON Should we say what N is??}
        \label{fig:pme_waves}
    }
\end{figure}
...
\comment{FROM ALISON: I rewrote the rest of this section - please check!}
These patches can be stable. For example, if
the density-dependent effects of individuals
extend farther, but not too much farther, than the typical dispersal distance,
...
of~\eqref{general deterministic limit}. However, it may not be unique,
it may be unstable, and a stable solution may have 
oscillations on a scale determined by the interaction distance.
...
\[
	\frac{\sigma^2}{\epsilon^2}v^2\left(-\varphi_0 r_0\gamma_0'
	-r_0\gamma_0e^{2\pi^2v^2}\right)+\varphi_0F_0',
\] 
...
= \frac{1}{2} G''(\langle f, \eta \rangle)
    \frac{\theta}{N}
	\int_{\IR^d}\left\{
		\gamma(x, \eta) \int f^2(y) r(y, \eta) q_\theta(x, dy) + f^2(x) \mu_\theta(x, \eta) \right\}
    \eta(dx) .
\end{align*}
\comment{FROM ALISON integral against eta in last line corrected}
...
we say that a ${\mathcal D}_{[0,\infty)}({\mathcal C}_N)$-valued process 
	\comment{FROM ALISON: we have not been consistent in the layout of 
	the notation for spaces of cadlag paths - it is already different in 
...
$\kappa^N(\lp)$.
\comment{ALISON: added N, also made sigma $\sigma$ above}
...
\begin{proposition}
	...
	\comment{ALISON deleted excess given}
...
\begin{theorem}
    ...
    in the sense of \eqref{eqn:conditionally_uniform}.
    Then, 
	\comment{FROM ALISON deleted `any sequence of stochastic processes'
	which didn't make sense - we want the ones already specified }
	$(\lp_t^N)_{t \ge 0}$
...
\comment{FROM ALISON: slightly reworded - deleted bit about onestep convergence
on the basis that we already point to that in the introduction}
...
\comment{BEGIN PREVIOUS VERSION}
We write
\begin{align*}
...
that depend on $f$ only through 
$\max_i \|\partial_{x_i} f\|_\infty$ and
...
\comment{FROM ALISON Does this merit being a subsubsection - if so I would expect 
some others too}
...
special case of $d=1$.
We take a sequence of test functions $f_n$ increasing to the function $f(x)=x$
...
we find that $\IE[\langle x, \eta_t(dx)\rangle] \le C'$
for $t\in [0,T]$, and an application of Markov's inequality
tells us that for any $\delta > 0$,
$\IP[\eta_t([R,\infty])>\delta]\to 0$ as $R\to\infty$.
The same argument with $f_n(x)$ increasing to $-x$ completes the proof.
\end{proof}
