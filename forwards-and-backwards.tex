...
\item For each $f \in C_b^2(\IR^d)$, there is a $C_f$ such that
...
\comment{TODO: the previous definition of $\Pgen^N$ was the following wrong thing; fixup below}
\begin{equation}
\begin{aligned}
    \text{ONLY RIGHT TO $O(1/N)$} \qquad
    \Pgen^N G_f(\eta)
    &:=
    G'(\langle f, \eta \rangle)
        \theta \bigg\langle
...
\comment{(beware the different definition of $f$ to that lemma)}
...
\begin{align}
    ...
    \bigg\rangle
\end{align}
The generator of $\lp^N_t$ is $A^N$, defined in equation~\label{eqn:lookdown_generator}.
