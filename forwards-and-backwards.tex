...
\begin{definition}[Model parameterization and assumptions]
    ...
    \begin{align}
        ...
        \gamma(\smooth{\gamma} \eta^{\theta}(x))
        - \frac{1}{\theta}
        F(x, \smooth{F} \eta^{\theta}(x)) .
    \end{align}
...
$$
    \gamma(x, \smooth{\gamma} \eta^{\theta}(x))
    \int r(x, \smooth{r} \eta^{\theta}(y)) q(x, dy) ,
$$
...
$$
    ...
    :=
    \frac{
        r(y, \smooth{r} \eta^{\theta}(y)) q_\theta(x, dy)
    }{
...
\begin{theorem} \label{thm:nonlocal_convergence}
    ...
    \begin{align} \label{eqn:mgale_problem}
        ...
        -
        \int_0^t \big\langle
            \gamma(x, \smooth{\gamma} \eta^{\theta}_{s}(x))
            \mathcal{B}\left(
                f(\cdot) r(\cdot, \smooth{r} \eta^{\theta}_{s}(\cdot))
            \right)(x)
            +
            f(x)
            F(x, \smooth{F} \eta^{\theta}_{s}(x)),
            \eta^{\theta}_{s}(dx)
        \big\rangle ds,
    \end{align}
    is a martingale.
    ...
    \begin{align} \label{eqn:mgale_variation}
        ...
        \int_0^t
        \big\langle
            \gamma\left( x, \smooth{\gamma} \eta^{\theta}_{s}(x) \right)
            r\left(x, \smooth{r} \eta^{\theta}_{s}(x) \right)
            f^2(x),
            \eta^{\theta}_{s} (dx)
        \big\rangle ds. 
    \end{align}
...
the scaled population model with levels where 
$\lambda$ is the scaling factor of the mass of an individual,
$\theta(\lambda)$ is the scaling factor for time,
and the level of an individual particle must be in $[0,\lambda]$.
Recall that we only track the adult population; maturity, if achieved, is instantaneous.
Furthermore, at rate $\theta\gamma(x, \eta)$,
a parent gives birth to a single juvenile offspring.

We should consider an initial population of $O(\lambda)$ many particles with levels
uniformly distributed between $[0, \lambda]$.
We write $(\xi^{\theta, \lambda}(t))_{t \geq 0}$
for our population at the $\theta$-th stage of rescaling.
To recover the scaling in \S\ref{sec: Model and main results},
we scale the empirical distribution of particles with a factor of $1/\lambda$,
so we write
$$\eta^{\theta, \lambda}_t:=\frac{1}{\lambda}\xi^{\theta, \lambda}_t(dx \times [0,\lambda])=
...
produces one adult offspring at rate $\theta\gamma^{\mathfrak{m}}_{\theta}(x, \eta)$, where
$$\gamma^{\mathfrak{m}}_{\theta}(x,\eta)=\gamma(x,\eta) \int r (y, \eta)q_\theta(x,dy).$$

The location of this offspring is given by
the distribution 
\[ q^{\mathfrak{m}}_\theta(x,\eta,dy) = \frac{r(y,\eta) q_{\theta}(x,dy)}{\int_{\mathbb{R}^d} r(y,\eta) q_{\theta}(x,dy)}. \]

In the lookdown version of our model,
the individual $(x,u)\in \xi^{\theta, \lambda}$
gives birth to an offspring at rate 
$2(\lambda-u)\lambda^{-1}\theta \gamma^{\mathfrak{m}}_{\theta}(x, \eta)$. 
(Averaging over the distribution of the
level $u$,
we recover $\theta \gamma^{\mathfrak{m}}_{\theta}(x, \eta)$.)
...
$$\dot{v}= \theta \gamma^{\mathfrak{m}}_{\theta}(x, \eta) 
...
$$\dot{u}= \theta \gamma^{\mathfrak{m}}_{\theta}(x, \eta) 
\left\{\lambda^{-1}(\lambda -u)^{2}
-(\lambda -u)\right\}+\theta \mu(x,\eta)u.$$
We shall write 
$$b_{\theta, \lambda}(x,\eta):=\theta\big\{\gamma^{\mathfrak{m}}_{\theta}(x,\eta)
-\mu(x,\eta)\big\}.$$
...
\begin{align}
\dot{u}&=\lambda \theta \gamma^{\mathfrak{m}}_{\theta}(x,\eta) 
\left\{\left(1-\frac{u}{\lambda}\right)^{2}
-1 +2\frac{u}{\lambda}\right\} -b_{\theta, \lambda}(x,\eta)u \nonumber \\
&=\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\theta}(x,\eta) u^2 -b_{\theta, \lambda}(x,\eta)u. \label{differential equation for level}
\end{align}
In this setting, the generator of our model
acting on the test functions $f(\eta)=\prod_{(x,u) \in \eta}g(x,u)$
satisfies
%We assume that  locations in ${\mathbb{ R}}^d$.

\begin{eqnarray}\label{Generator of prelimit simplest model}
&&A_{\theta, \lambda}f(\eta ) \nonumber \\
&&\quad =f(\eta )\,\sum_{(x,u)\in\eta}\,2\lambda^{-1}\theta \gamma^{\mathfrak{m}}_{\theta}(x, \eta)
\int_u^{
\lambda}\Bigg(\frac 12\frac {\int_{\mathbb{R}^d}g(y,u)q^{\mathfrak{m}}_\theta(x,\eta,d
y)g(x,v_1)}{g(x,u)} \label{multgen3}\\
&&\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad
+\frac 12\int_{\mathbb{R}^d}g(y,v_1)q^{\mathfrak{m}}_\theta(x,\eta ,dy)-
1\Bigg)dv_1\nonumber\\
&&\quad \quad +f(\eta )\left(\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\theta}(x,\eta) u^2 -b_{\theta, \lambda}(x,\eta)u\right)\frac {\partial_u g(x,u)}{g(x,u)}\nonumber\\
&&\quad =f(\eta )\,\sum_{(x,u)\in\eta}\,2 \gamma^{\mathfrak{m}}_{\theta}(x, \eta)\lambda^{-1}\int_u^{
\lambda}\Bigg(\frac 12\frac {\theta\int_{\mathbb{R}^d}(g(y,u)-g(x,u))q^{\mathfrak{m}}_\theta(x,\eta,dy)g(x,v_1)}{g(x,u)}\non\\
&&\qquad\qquad\qquad\qquad\qquad\qquad\qquad +\theta\int_{\mathbb{R}^d}\left(\frac{g(y,v_1)+g(x,v_1)}{2}-1\right)q^{\mathfrak{m}}_\theta(x,\eta,dy)\Bigg)dv_1\nonumber\\
&&\qquad\qquad
+f(\eta )\sum_{(x,u)\in\eta}\,\left(\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\theta}(x,\eta) u^2 -b_{\theta, \lambda}(x,\eta)u\right)\frac {\partial_u g(x,u)}{g(x,u)}.\nonumber
\end{eqnarray}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection*{Scaling of the Pre-limit}

We also let $\lambda$ takes on the role of $N$
in \S \ref{sec: Model and main results},
i.e. each individual has mass $1/\lambda$
in the scaling limit.
Time is once again scaled by $\theta$
and local interaction radius by $\varepsilon_{\gamma}$ and $\varepsilon_{r}$.

As in Definition \ref{Def: Rescaled Models},
the rates $\gamma,r, \mu_{\theta}$
in our re-scaled lookdown models 
depend on the local density of $\eta$
through mollifications with 
$\varrho_{\gamma}, \varrho_{r}$ and $\varrho_{F}$.
We will now make our re-scaled lookdown model explicit.

\begin{definition}[Martingale Problem Characterisation]
\label{Def: Rescaled Lookdown Models}
Let the coefficients $\gamma, \mu, r$ satisfy the conditions highlighted in Definition \ref{Def: Rescaled Models} and let $\varrho_{\varepsilon}$ be the kernel defined in Definition \ref{Defn: Kernel and Convolution}. We define the following terms:
\begin{align*}
\gamma^{\mathfrak{m}}_{\varepsilon,\theta}(x, \eta):=&~\gamma(x, \varrho_{\varepsilon}*\eta^{\theta}(x)) \int r(y, \varrho_{\varepsilon}*\eta^{\theta}(y))q_{\theta}(x,dy),\\
 q^{\mathfrak{m}}_{\varepsilon, \theta}(x,\eta,dy) :=&~ \frac{r(y,\varrho_{\epsilon}*\eta^{\theta}(y)) q_{\theta}(x,dy)}{\int_{\mathbb{R}^d} r(y,\varrho_{\epsilon}*\eta^{\theta}(y)) q_{\theta}(x,dy)},\\
b_{\varepsilon, \theta, \lambda}(x, \eta):=&~ \theta \bigg\{ \gamma(x, \varrho_{\varepsilon}*\eta^{\theta}(x)) \int r(y, \varrho_{\varepsilon}*\eta^{\theta}(y))q_{\theta}(x,dy)-\mu_{\theta}(x, \varrho_{\varepsilon}*\eta^{\theta}(x)) \bigg\}
\end{align*}
At the $\lambda$-stage of re-scaling, our test measures $\eta$ takes value in $\mathcal{M}_F(\mathbb{R}^d \times [0,\lambda])$, the set of finite measures on $\mathbb{R}^d \times [0,\lambda]$, and we define $$\eta= \frac{1}{\lambda}\sum_{(x,u) \in \eta}\delta_x.$$
...
&&\quad =f(\eta )\,\sum_{(x,u)\in\eta}\,2 \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(x, \eta)\Bigg\{ \frac 12 \lambda^{-1}\int_u^{
...
\lambda}\int_{\mathbb{R}^d}\left(\frac{g(y,v_1)+g(x,v_1)}{2}-1\right)q^{\mathfrak{m}}_{\varepsilon,\theta}(x,\eta,dy)dv_1\nonumber\Bigg\}\\
&&\qquad\qquad
+f(\eta )\sum_{(x,u)\in\eta}\,\left(\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(x,\eta) u^2 -b_{\varepsilon,\theta, \lambda}(x,\eta)u\right)\frac {\partial_u g(x,u)}{g(x,u)}.\nonumber
\end{eqnarray}
...
$$(\eta^{\varepsilon,\theta, \lambda}(t))_{t \geq 0}:=\left(\frac{1}{\lambda}\sum\limits_{\substack{(X(t),U(t))\in \eta^{\varepsilon,\theta, \lambda}(t) \\ U(t) < \lambda}} \delta_{X(t)}\right)_{t \geq 0}$$
has the same distribution (as stochastic processes) as $(\eta^{\theta}_t)_{t \geq 0}$ in Definition \ref{Def: Rescaled Models}. As a result, we use the same notation $\eta$ to denote these two essentially identical processes which take values in the space of empirical measures. On the other hand, we reserve the notation $\eta$ strictly for the spatial-level process.\\~\\ Furthermore, the application of Markov Mapping Theorem implies that particle levels are, conditional on $\eta$, i.i.d and uniformly distributed between $[0, \lambda]$.  Theorem A.10 in \cite{kurtz/rodrigues:2011} states that convergence of $(\eta^{\varepsilon, \theta, \lambda}_t)_{t \geq 0}$ to a conditionally Poisson system $(\xi_t)_{t \geq 0}$ with Cox measure $\Xi_t \times \Lambda$ is equivalent to convergence of $(\eta^{\varepsilon, \theta, N}_t)_{t \geq 0}$ to $(\Xi_t)_{t \geq 0}$. As a result, it suffices for us to focus on tightness of $(\eta^{\varepsilon, \theta, \lambda}_t)_{t \geq 0}$ and spatial convergence of $(\eta^{\varepsilon, \theta, N}_t)_{t \geq 0}$ as $\theta \to \infty$.
...
Now $U_{\tau}$ satisfies
\begin{equation}
U_{\tau}(t)=U_{\tau}(\tau)+\int_{\tau}^{t}(\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(s),\eta(s))U^2_{\tau}(s)-b_{\varepsilon,\theta,\lambda}(X_{\tau}(s),\eta(s))U_{\tau}(s))ds, ~~~ t \geq \tau.    
\end{equation}
...
\begin{equation}\label{New birth potential level}
    U_{\tau+s}(\tau+s)=U_{\tau}(\tau+s)+ \frac{v}{2\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(\tau+s-), \eta(\tau+s-))}.
\end{equation}
...
X_{\tau}(\tau+t)=&X_{\tau}(t)\\
&+\int_{(0,t]\times[0,\infty]\times[0,1]\times\{0,1\}}\mathbbm{1}_{U_{\tau}(\tau+s)+ \frac{v}{2\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(\tau+s-), \eta(\tau+s-))}<\lambda\}}\kappa    H(X_{\tau}(\tau+s-), \eta(\tau+s),\omega) \\
...
\begin{align*}
U_{\tau}(t)=&U_{\tau}(0)+\int_{0}^{t}\frac{1}{2\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(s-), \eta(s-))}dV_{\tau}(s)\\
&+\int_{0}^{t}\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(s-), \eta(s-))U^2_{\tau}(s)-b_{\varepsilon,\theta, \lambda}(X_{\tau}(s),\eta(s))U_{\tau}(s))ds,\\  
X_{\tau}(t)=&X_{\tau}(0)+\int_{(0,t]\times[0,\infty]\times[0,1]\times\{0,1\}}\mathbbm{1}_{U_{\tau}(\tau+s)+ \frac{v}{2\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(\tau+s-), \eta(\tau+s-))}<\lambda\}}\\
...
\begin{remark}
The factor $\frac{1}{2\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(x, \eta)}$ that scales the jump in levels give an average birth rate of $\theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(s-), \eta(s-))$. This is because for the potential birth to be a true birth, we impose that $$U_{\tau}(\tau+s)+ \frac{v}{2\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(\tau+s-), \eta(\tau+s-))}< \lambda,$$ where $U_{\tau}(\tau+s)$ is uniformly distributed between $[0, \lambda]$. As a result, a true birth occurs between $[0,\delta t]$ if there exists $(s,v) \in [0,\delta t]\times [0, 2 (\lambda-U_{\tau}(\tau+s)) \lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(\tau+s-), \eta(\tau+s-))]$. Averaging this over levels give a Poisson process with rate $ \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(\tau+s-), \eta(\tau+s-))$. 
\end{remark}
\begin{remark}
...
\begin{equation}
\begin{aligned}
\lambda^{-1} \theta \gamma^{\mathfrak{m}}_{\varepsilon,\theta}(X_{\tau}(s), \eta(s)) &\to \ell \gamma(X_{\tau}(s),\varrho_\varepsilon*\Xi(X_{\tau}(s)),\\
b_{\varepsilon,\theta, \lambda}(X_{\tau}(s),\eta(s)) &\to  F(X_{\tau}(s), (\varrho_{\varepsilon}*\eta)(X_{\tau}(s)))\\
