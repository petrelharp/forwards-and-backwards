...
\comment{TODO Should we worry that we used prime for differentiation before?}
...
\comment{TODO I think we need to make the notation for $r(y, \rho_r*\eta(y))$ 
consistent with what we have (now) used elsewhere, namely $r(y,\eta)$?}
...
\end{proof}
\comment{ALISON: Actually the last calculation gives a much better bound which we will
need later. I'll sort this out tomorrow.}
...
\comment{ALISON: It's actually using the bound derived just below (7.4) which is 
much stronger than stated there}
    \begin{align*}
        \IE\left[
...
\end{align} 
\comment{MISSING A TERM? the $\int_{s-\delta}^s$ one; and say it's bounded because of the moment bounds}
...
for any fixed $\delta>0$, if $s > \delta$ the terms arising from 
