...
\comment{Do we want subscript $a$s here, on e.g., $\zeta$?
I think not, because this is all given a particular $a$?}
...
which is easier to define if we treat point processes as sets for a moment,
defining
\begin{align*}
    \Gamma_a
    &=
    \bigcup_{k=1}^n 
    \bigg\{
        (s, v, y, \kappa) \in \Pi_{(a)_k} \; : \;
        \tau_{(a)_{k-1}} \le s < \tau_{(a)_{k-1}} 
    \\& \qquad \qquad 
        \text{ and }
        U_{(a)_k}(\tau_{(a)_{k-1}} + s) + \ell( X_{(a)_k}(\tau_{(a)_{k-1}} + s-), y, \eta(\tau_{(a)_{k-1}} + s-), v) < \lambda
    \bigg\}
\end{align*}
\comment{TODO include the switch points}

which is for $A \subset [0,\infty)^2 \times \IR^d \times \{0,1\}$
\comment{This is wrong, as it includes non-viable individuals.}
\begin{align*}
\Gamma_a(A)
    &=
    \sum_{k=1}^n
...
\end{align}
and
\begin{align} \label{eqn:U_line_of_descent}
    U_a(t)
    &=
    U_{a_1}(0)
    +
...
Now, consider equations~\eqref{eqn:X_line_of_descent} and~\eqref{eqn:U_line_of_descent}.

Equation~\eqref{eqn:X_line_of_descent} simply says that $X_a$ follows a random walk
