...
\comment{Should we just use $N$ instead of $\lambda$? Or vice-versa? Change to $N$ but comment that when comparing ot other papers set $\lambda=N$.}
...
\begin{align*}
    \eta_t = \frac{1}{\lambda} \sum_{a : \tau_a > t \;\&\; U_a(t) < \lambda} \delta_{X_a(t)}
    \qquad \text{and} \qquad
...
\begin{align*}
    \eta_t = \lim_{\lambda \to \infty} \frac{1}{\lambda} \sum_{a : \tau_a > t \;\&\; U_a(t) < \lambda} \delta_{X_a(t)}
    \qquad \text{and} \qquad
    \lp = \sum_{a : \tau_a > t} \delta_{(X_a(t), U_a(t))} .
\end{align*}
...
\mathbb{E}\left[
\left(
F_f(\eta^{\theta}_{t+\tau})-F_f(\eta^{\theta}_t)
-\int_{t}^{t+\tau}\Pgen^{\infty}F_f(\eta^{\theta}_s)ds
\right)
...
\begin{equation}
\begin{aligned}
&\mathbb{E}\left[F_f(\eta^{\theta}_{t+\tau})-F_f(\eta^{\theta}_t)\bigg| \mathcal{F}_t \right]\\
=& F'(\langle f, \eta^{\theta}_t \rangle )
\mathbb{E}\left[
                \langle f, \eta^{\theta}_{t+\tau}\rangle
                    -\langle f, \eta^{\theta}_t\rangle 
                    \bigg| \mathcal{F}_t
         \right]
...
\mathbb{E}\left[
                \left(
                    \langle f, \eta^{\theta}_{t+\tau}\rangle -\langle f, \eta^{\theta}_t\rangle
                \right)^2
                \bigg| \mathcal{F}_t
        \right]\\
&+\frac{1}{6}F'''(\langle f, \eta^{\theta}_{t+\tau'}\rangle)\mathbb{E}\left[
                \left(
                    \langle f, \eta^{\theta}_{t+\tau}\rangle -\langle f, \eta^{\theta}_t\rangle
                \right)^3
                \bigg| \mathcal{F}_t
...
& \times  \int_{t}^{t+\tau}\bigg\{
    ...
    \bigg\} ds\\
&+\frac{1}{2}F''(\langle f, \eta^{\theta}_t \rangle )\\
&\quad \times \frac{\theta}{N}\int_{t}^{t+\tau}\bigg\{
\langle \gamma\big(x,\rho_{\gamma}*\eta^{\theta}_{s}(x)\big)
\int f^2(y)r\big(y,\rho_{r}*\eta^{\theta}_{s}(y)\big)q_\theta(x,dy) 
,\eta^{\theta}_{s}(dx)\rangle\\
& \qquad \qquad +\langle \mu\big(x,\eta^{\theta}_{s}\big)f^2(x) 
,\eta^{\theta}_{s}(dx)\rangle \bigg\}ds\\
&+\frac{1}{6}F^{(3)}(\langle f, \eta^{\theta}_{t+\tau'}\rangle)\mathbb{E}\left[
                \left(
                    \langle f, \eta^{\theta}_{t+\tau}\rangle -\langle f, \eta^{\theta}_t\rangle
                \right)^3
                \bigg| \mathcal{F}_t
...
We first show that 
$$\lim_{\theta \to \infty}\mathbb{E}\left[
                \left|
                    \langle f, \eta^{\theta}_{t+\tau}\rangle -\langle f, \eta^{\theta}_t\rangle
                \right|^3
                \bigg| \mathcal{F}_t
        \right] \to 0.$$
Since the total number of births and deaths
between $[t,t+\tau]$ is given by Poisson point processes
with rates $\theta \gamma\big(x,\rho_{\gamma}*\eta^{\theta}_{s}(x)\big)$
and $\theta \mu\big(x,\eta^{\theta}_{s}\big)$
respectively,
and each birth and death events contribute
an increment of order $\mathcal{O}(1/N^3)$,
we have that
\begin{multline}
\lim_{\theta \to \infty}\mathbb{E}\left[
                \left|
                ...
                \bigg| \mathcal{F}_t
        \right] \\
\leq ||f||_{\theta} \frac{\theta}{N^2}\int_{0}^{t}\left\langle\gamma\big(x,\rho_{\gamma}*\eta^{\theta}_{s}(x)\big)+\mu\big(x,\eta^{\theta}_{s}\big), \eta^{\theta}_s(dx)\right\rangle ds \to 0
\end{multline}
as $\theta \to \infty$.
\end{proof}
