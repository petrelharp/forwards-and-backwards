...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
    ...
    (Lemma~\ref{lem:compact containment lookdown}),
    ...
    and $h \in C_c([0,\infty))$ is continuous with compact support.
...
Let $C_c^+$ denote the set of nonnegative continuous functions on $\overline{\IR^d} \times [0, \infty)$
for which there exists a $u_f \ge 0$ such that $u > u_f$ implies that $f(x, u) = 0$.
Recall that we are working with the vague topology ``in the level direction'',
so that $m_n \to m$ in $\lpmeasures$ if and only if $\langle f, m_n \rangle \to \langle f, m \rangle$
for all $f \in C_c^+$.
By Theorem~A.23 in \citet{kallenberg1997foundations},
the space $\lpmeasures$ is separable and metrizable,
and $K \subset \lpmeasures$ is relatively compact if and only if
$\sup_{\lp \in K} \langle f, \lp \rangle < \infty$
for any $f \in C_c^+$.
% The compact sets we consider are those with bounded total mass of their spatial projection,
% i.e., of the form
% $$
%     K_M = \left\{
%         \lp \in \lpmeasures \;:\;
%         \sup_{L \ge 1} \frac{1}{L} \lp\left(\overline{\IR^d} \times [0, L]\right) \le M
%     \right\} .
% $$
% These sets are compact 
% because if $\lp \in K_M$ and $f \in C_c^+$ then $\langle f, \lp\rangle \le M u_f \|f\|_\infty$.
Let $\mathcal{H} = \{h_n\}_{n \ge 1}$ be a sequence of
monotone, nonnegative, compactly supported, differentiable functions
on $[0, \infty)$ such that there exist sequences $u_n \to \infty$ and $k_n$ such that
\begin{enumerate}
    \item $h_n(u) > 1$ for $u \le u_n/2$,
    \item $h_n(u) = 0$ for $u \ge u_n$,
    \item $- h_n(u) \le h_n'(u) \le 0$, and
    \item $\max\left( \int_u^\infty h_n(v) dv, \int_u^\infty h_n(v)^2 dv \right) \le k_n h_n(u)$.
\end{enumerate}
For instance, we could take $h_n(u)$ to be a mollified version of $\min(1, e^{-(u-n)})$,
and then $u_n = k_n = n$.
\comment{(or we could just take those functions? they are differentiable almost everywhere.)}
The compact sets we consider are those with uniformly bounded integrals against $\mathcal{H}$,
i.e., of the form
$$
    K_C = \left\{
        \lp \in \lpmeasures \;:\;
        \langle h_n, \lp \rangle \le C_n
        \text{ for all } n \ge 1
    \right\} ,
$$
for some sequence of constants $(C_1, C_2, \ldots)$.
This set is compact because any $f \in C_c^+$ is bounded by a constant multiple of some $h_n$.
...
\begin{lemma}[Compact containment for $\lp$]
    ...
    \comment{and they are conditionally Poisson},
    ...
    \limsup_{N \to \infty}
    \IP\left\{
        \sup_{0 \le t \le T} \langle h_n, \lp^N_t \rangle > C_n
        \text{ for some } n \ge 1
    \right\}
    < \delta .
    \end{align}
...
In other words, the processes $\lp^N$ stay in $K_C$ for all $0 \le t \le T$
with uniformly high probability.
We will prove this Lemma using the following one:

\begin{lemma}
    \label{lem:compact containment hn}
    Under the assumptions of Lemma~\ref{lem:compact containment lookdown},
    for any $T$, $M$ and $n$ there exists a constant $K_n$ such that
    $$ 
    \limsup_{N \to \infty} \IP\left\{ \sup_{0 \le t \le T} \langle h_n, \lp^N_t \rangle > M \right\}
    <
    \frac{K_n}{M} .
    $$
\end{lemma}
...
\begin{proof}[of Lemma~\ref{lem:compact containment lookdown}]
    ...
    by taking $C_n = 2^{n-1} K_n / \delta$ and using Lemma~\ref{lem:compact containment hn}.
\end{proof}

\begin{proof}[of Lemma~\ref{lem:compact containment hn}]
    \comment{TODO: just give bounds and say this follows
        the proof of Lemma~\ref{lem:eta_compact_containment} exactly, otherwise.
        Or make the basic idea a lemma in the appendix?}
    Let $h_n \in \mathcal{H}$.
    It is evident from the description of the process
    (or, by differentiating in Definition~\ref{defn:lookdown_mgale})
    that
    Applied to $f(x, u) = h_n(u)$, the martinagle representation~\eqref{eqn:f_xi_mgale} is
    \begin{align*}
    \begin{split}
        \langle h_n, \lp_t^N \rangle
        &=
        \langle h_n, \lp_0^N \rangle
        + M^{h_n}_t 
        \\ & \qquad \qquad {}
        +
        \int_0^t
        \big\langle
            2 a^N(x, \eta^N_s) \int_u^N h_n(u_1) du_1
        ,
    ...
    \end{split}
    \end{align*}
    where $M^{h_n}_t$ is a martingale
    with quadratic variation
    \begin{align*}
        [M^{h_n}]_t
        &=
        \int_0^t
        \langle 2 a^N(x, \eta_s^N) \int_u^N h_n(u_1)^2 du_1
        ,
        \lp_s^N(dx, du) \big \rangle
        ds .
    \end{align*}
    Now, note that $0 \le a^N(x, \eta_x^N) \le C_a$ for some $C_a < \infty$,
    and we have assumed that $\int_u^\infty h_n(u_1) du_1 \le k_n h_n(u)$,
    so that 
    \begin{align*}
        & \int_0^t
        \big\langle
        ...
        \\ & \qquad
        \le
        2 C_a k_n \int_0^t \big\langle h_n(u), \lp_s^N(dx, du) \big \rangle .
    \end{align*}
    In the next line, the contribution from $a^N(x, \eta^N_s) u^2 h'(u)$ is nonpositive because
    $a^N \ge 0$ and $h'(u) \le 0$.
    Similarly, since $b^N(x, \eta_x^N) \le C_b$ for some $C_b < \infty$,
    and by assumption
    $- h_n'(u) \le k_n h_n(u)$ and $h_n'(u) = 0$ for $u \ge u_n$,
    and so
    \begin{align*}
        & \int_0^t
        \big\langle
        ...
        ds
        \le
        C_b u_n k_n \int_0^t \big\langle h_n(u), \lp_s^N(dx, du) \big \rangle .
    \end{align*}
    ...
    \begin{align} \label{eqn:hn_bound}
        ...
        \langle h_n, \lp_0^N \rangle
        + M^{h_n}_t 
        + (2 C_a + C_b u_n) k_n \int_0^t 
            \langle h_n, \lp_s^N \rangle
        ds .
    \end{align}
    By Gronwall's inequality, with $C = (2 C_a + C_b u_n) k_n$
    \begin{align*}
        \IE\left[ \langle h_n, \lp_t^N \rangle \right]
        &\le
        \IE\left[ \langle h_n, \lp_0^N \rangle \right] e^{Ct} .
    \end{align*}

    Now consider the maximum. By \eqref{eqn:hn_bound},
    \begin{align*}
        \sup_{0 \le t \le T} \langle h_n, \lp_t^N \rangle
        &\le
        \langle h_n, \lp_0^N \rangle
        + \sup_{0 \le t \le T} M^{h_n}_t 
        + C \int_0^t 
            \sup_{0 \le t \le T} \langle h_n, \lp_s^N \rangle
        ds .
    \end{align*}
    ...
    \begin{align*}
    \IE\left[ \sup_{0 \le t \le T} M^{h_n}_t \right]
    & \le
        C'\left( 1 + \IE\left[ [M^{h_n}]_t \right] \right)
    \\ & \le
        ...
        \right] ds \right) 
    \\ & \le
        C' \left( 1 + 2 C_a k_n \int_0^t \IE\left[
        \langle h_n, \lp_s^N \big \rangle
        \right] ds \right) 
    \\ & \le 
        C' \left( 1 + C'' e^{Ct} \IE[ \langle h_n, \lp_t^N \rangle ] \right) ,
    \end{align*}
    where $C'' = 2 C_a k_n / C$.
    Therefore, 
    \begin{align*}
        \IE\left[ \sup_{0 \le t \le T} \langle h_n, \lp_t^N \rangle \right]
        &\le
        C' + \IE\left[ \langle h_n, \lp_0^N \rangle \right]( 1 + C'' e^{Ct} )
        + C \int_0^t 
            \IE\left[ \sup_{0 \le t \le T} \langle h_n, \lp_s^N \rangle \right]
        ds ,
    \end{align*}
    and so once again by Gronwall's inequality, there is a $C'''$ such that
    \begin{align*}
        \IE\left[ \sup_{0 \le t \le T} \langle h_n, \lp_t^N \rangle \right]
        &\le
        C'''(1 + \IE[\langle h_n, \lp_0^N \rangle]) e^{2Ct} ,
    \end{align*}
    and so
    \begin{align*}
...
\begin{proof}[of Lemma \ref{lem:test_fn_tightness}]
    ...
    Lemma~\ref{lem:eta_projections_tightness},
    except that terms are bounded by constant multiples of $\langle h_n, \xi^N_s \rangle$
    instead of $\langle 1, \eta^N_s \rangle$,
    where $h_n$ is bounded below on $[0, u_*]$.
    \comment{(or we could say that $u_n \ge 2 u_*$)}
    ...
    \end{align*}
    but this is now clearly bounded by a constant multiple of $h_n(u)$.
\end{proof}
