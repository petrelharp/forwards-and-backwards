...
\vspace{.1in}
    ...
    \comment{make notation for $\mathcal{D}$ consistent}
...
\comment{FIX UP THIS, REFER FORWARD}
...
Condition~\ref{control through r} is also satisfied if for example
...
\begin{figure}
	...
	$\alpha =\theta/N$ (right).
        ...
        Blue curves are contours from a kernel density estimate.
...
porous medium equation with logistic growth~(\ref{PME}), comparing the simulated
        ...
        % (Note: noisier wave should move slower, although maybe not much since it's pushed.)
        \label{fig:pme_waves}
    }
\end{figure}
...
\begin{proof}[of Proposition~\ref{prop:limiting_construction}]
    ...
    \comment{label the system and refer to it}
...
then the fact that $0 \le r \le 1$,
there is a $\kappa$ in $[0, 1]$ such that the first term is
\begin{align*}
    &
    \left| \theta 
        \int_{\IR^n} 
    ...
    \left| \theta 
        \int_{\IR^n} 
            \left(
                ...
                +
                \frac{1}{2} \sum_{ij} (y-x)_i (y-x)_j \partial_{x_i x_j} f(x + \kappa(y-x))
            \right)
            r(y, \eta)
        q_{\theta}(x,dy)
    \right|
...
\comment{TODO: define $\mathcal{P}^N$ also here.}
...
\begin{proof}[of Proposition~\ref{thm:mmt_application}]
    ...
    \comment{TODO: copy the MMT into our appendix and refer to it}
...
\begin{proof}[Proof of Theorem~\ref{thm:lookdown_convergence}]
    ...
    The analogue of Lemma~\ref{lem:limit_mgale},
    in particular that the martingale properties are preserved under passage to the limit,
    are proved in Section~\ref{sec:lookdown_generator_proofs}.
