...
\comment{Below, we write $\lambda$ instead of $N$: change to $N$?}
...
in \citet{etheridge/kurtz:2018} and \citet{kurtz/rodrigues:2011}.
...
\comment{This repeats some stuff from above.}
...
where $q(x,dy)$ is the dispersal kernel.

\comment{Note: in what happens below, $\gamma^\mathfrak{m}$ is useful in that it makes the
equation for $\dot u$ nicer, but we could remove $q^\mathfrak{m}$ if we wanted
(although then \eqref{Generator of prelimit simplest model}
would then have both a $\gamma$ and a $\gamma^\mathfrak{m}$ in it).}
...
\comment{TODO: explain the connection to $F$.}
In this notation,
the differential equation 
governing the level of the individual $(x,u)\in\eta$ is
\comment{TODO: check that $\theta$ is right here, and consistent with the discussion above.}
\begin{align}
