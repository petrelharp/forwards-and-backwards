...
After liberal use of the product rule, we'll be able to simplify this last expression.
To prepare for this, first define
$$
...
\comment{Note here we are allowing $\meanq$ to vary with space!}

Now consider the adjoint of $\mathcal{L}^Y_s$.
Recall that $\DG g = \DD g + \meanq \cdot \grad g$
and $\DG^* f = \DD f - \grad \cdot (f \meanq)$,
so
\begin{align*}
    ({\mathcal L}^Y_s)^* f
    &=
    ...
    &=
    r \DD(\gamma f) - r \grad \cdot (\gamma f \meanq)
    - \gamma f \DG r .
\end{align*}
We can now expand the expression for $\hat{\mathcal L}_s$ to get
\begin{eqnarray*}
\hat{\mathcal L}_s g
    &=&
    \frac{1}{\varphi_s} \left[
        r \DD(\gamma \varphi_s g) - r \grad \cdot (\gamma \varphi_s g \meanq)
        - \gamma \varphi_s g \DG(r)
        + \gamma \varphi_s g \DG(r)
        - r g \DG^*(\gamma \varphi_s)
    \right] \\
    ...
    \right] ,
\end{eqnarray*}
in agreement with \eqref{eqn:heuristic_lineage_generator}.
